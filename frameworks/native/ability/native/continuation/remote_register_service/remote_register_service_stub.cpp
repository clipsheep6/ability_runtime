...
{
    HILOG_DEBUG("%{public}s called, cmd=%{public}d, flags=%{public}d", __func__, code, option.GetFlags());

    std::u16string descriptor = IRemoteRegisterService::GetDescriptor();
    std::u16string remoteDescriptor = data.ReadInterfaceToken();
    if (descriptor != remoteDescriptor) {
        HILOG_DEBUG("%{public}s local descriptor is not equal to remote", __func__);
