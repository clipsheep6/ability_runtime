...
void ConnectCallbackProxy::RemoteRequest(MessageParcel &data, int commandDisconnect)
{
    ...
    int32_t ret = remote->SendRequest(commandDisconnect, data, reply, option);
    if (ret != NO_ERROR) {
        HILOG_ERROR("SendRequest failed. cmd is %{public}d, ret is %{public}d.", commandDisconnect, ret);
        return;
    }
    HILOG_INFO("%{public}s called end", __func__);
}
