...
{
    HILOG_DEBUG("%{public}s called begin. progressState_: %{public}d, newState: %{public}d",
        __func__,
        progressState_,
        newState);
...
bool ContinuationManager::CheckAbilityToken()
{
    ...
    if (continueToken_ == nullptr) {
        HILOG_DEBUG("%{public}s called failed", __func__);
        return false;
    }
...
bool ContinuationManager::DoScheduleStartContinuation()
{
    ...
    if (!ability->OnStartContinuation()) {
        HILOG_DEBUG("%{public}s called failed to StartContinuation", __func__);
        return false;
    }
    HILOG_DEBUG("%{public}s called end", __func__);
    return true;
