...
void CJUIAbility::OnStart(const Want &want, sptr<AAFwk::SessionInfo> sessionInfo)
{
    ...
    if (!cjAbilityObj_) {
        TAG_LOGE(AAFwkTag::UIABILITY, "cjAbilityObj_ is null");
        return;
    }
...
void CJUIAbility::OnSceneRestored()
{
    ...
    auto delegator = AppExecFwk::AbilityDelegatorRegistry::GetAbilityDelegator();
    if (delegator) {
        TAG_LOGD(AAFwkTag::UIABILITY, "call delegaotr PostPerformScenceRestored");
...
void CJUIAbility::DoOnForeground(const Want &want)
{
    ...
    if (scene_ == nullptr) {
        ...
        auto window = scene_->GetMainWindow();
        if (window  == nullptr) {
            TAG_LOGE(AAFwkTag::UIABILITY, "MaiwindownWindow is nullptr");
            return;
        }
...
void CJUIAbility::ContinuationRestore(const Want &want)
{
    ...
    if (!IsRestoredInContinuation() || scene_ == nullptr) {
        TAG_LOGE(AAFwkTag::UIABILITY, "is not restore in continuation or scene_ is nullptr");
        return;
    }
    RestorePageStack(want);
    OnSceneRestored();
...
int32_t CJUIAbility::OnContinue(WantParams &wantParams)
{
    ...
    TAG_LOGD(AAFwkTag::UIABILITY, "end, return value: %{public}d", res);
...
void CJUIAbility::OnConfigurationUpdated(const Configuration &configuration)
{
    ...
    if (!cjAbilityObj_) {
        TAG_LOGE(AAFwkTag::UIABILITY, "cjAbilityObj_ is nullptr");
        return;
    }
