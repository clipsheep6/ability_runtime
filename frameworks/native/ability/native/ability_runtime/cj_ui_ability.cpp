...
{
    return new (std::nothrow) CJUIAbility(static_cast<CJAbilityRuntime &>(*runtime));
}

CJUIAbility::CJUIAbility(CJAbilityRuntime &cjAbilityRuntime) : cjAbilityRuntime_(cjAbilityRuntime)
{
    HILOG_DEBUG("Called.");
}
    ...
    const sptr<IRemoteObject> &token)
{
    ...
    if (abilityInfo == nullptr) {
        HILOG_ERROR("Ability Info is nullptr.");
        return;
    }
#ifdef SUPPORT_GRAPHICS
    if (abilityContext_ != nullptr) {
    ...
    if (!abilityInfo->isModuleJson) {
        HILOG_ERROR("abilityInfo is not module json.");
    ...
    const std::shared_ptr<AbilityInfo> &abilityInfo, const std::string &moduleName, const std::string &srcPath)
{
    ...
    cjAbilityObj_ = CJAbilityProxy::LoadModule(abilityInfo->name);
    if (!cjAbilityObj_) {
...
void CJUIAbility::DoOnForegroundForSceneIsNull(const Want &want)
{
...
const CJAbilityRuntime &CJUIAbility::GetCJAbilityRuntime()
{
    return cjAbilityRuntime_;
}
...
std::shared_ptr<CJAbilityProxy> CJUIAbility::GetCJAbility()
{
    HILOG_DEBUG("Called.");
