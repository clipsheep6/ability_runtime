...
int32_t JsUIAbility::OnShare(WantParams &wantParam)
{
    ...
    napi_value obj = jsAbilityObj_->GetNapiValue();
    if (!CheckTypeForNapiValue(env, obj, napi_object)) {
        TAG_LOGE(AAFwkTag::UIABILITY, "failed get ability napi value");
        return ERR_INVALID_VALUE;
    }
...
void JsUIAbility::OnStop(AppExecFwk::AbilityTransactionCallbackInfo<> *callbackInfo, bool &isAsyncCallback)
{
    ...
    if (!isAsyncCallback) {
        TAG_LOGE(AAFwkTag::UIABILITY, "failed call promise");
        OnStopCallback();
    }
    TAG_LOGD(AAFwkTag::UIABILITY, "end");
}
...
void JsUIAbility::onSceneDestroyed()
{
    ...
    if (scene_ != nullptr) {
        ...
        if (window != nullptr) {
            TAG_LOGD(AAFwkTag::UIABILITY, "un register displaymovelistener");
...
std::unique_ptr<NativeReference> JsUIAbility::CreateAppWindowStage()
{
    ...
    if (jsWindowStage == nullptr) {
        TAG_LOGE(AAFwkTag::UIABILITY, "failed to create jsWindowSatge");
        return nullptr;
    }
...
void JsUIAbility::DoOnForegroundForSceneIsNull(const Want &want)
{
    ...
    if (setting_ != nullptr) {
        ...
        if (flag && !strDisplayId.empty()) {
            ...
            TAG_LOGE(AAFwkTag::UIABILITY, "failed formatRegex: [%{public}s]", strDisplayId.c_str());
        }
    }
    ...
    if (ret != Rosen::WMError::WM_OK) {
        TAG_LOGE(AAFwkTag::UIABILITY, "failed init window scene");
        return;
    }
...
sptr<IRemoteObject> JsUIAbility::CallRequest()
{
    ...
    if (remoteCallee_ != nullptr) {
        TAG_LOGE(AAFwkTag::UIABILITY, "remoteCallee_ exist");
    ...
    if (!CheckTypeForNapiValue(env, obj, napi_object)) {
        TAG_LOGE(AAFwkTag::UIABILITY, "null jsAbilityObj_ napi value");
        return nullptr;
    }
...
bool JsUIAbility::CallPromise(napi_value result, AppExecFwk::AbilityTransactionCallbackInfo<> *callbackInfo)
{
    ...
    if (then == nullptr) {
        TAG_LOGE(AAFwkTag::UIABILITY, "failed get property: then");
        return false;
    }
    bool isCallable = false;
    ...
    if (dumpInfo != nullptr) {
        ...
        for (uint32_t i = 0; i < len; i++) {
            ...
            if (!ConvertFromJsValue(env, element, dumpInfoStr)) {
                TAG_LOGE(AAFwkTag::UIABILITY, "failed parse dumpInfoStr");
                return;
            }
...
bool JsUIAbility::CheckSatisfyTargetAPIVersion(int32_t version)
{
    ...
    if (!applicationInfo) {
        TAG_LOGE(AAFwkTag::UIABILITY, "null targetAPIVersion applicationInfo");
        return false;
    }
