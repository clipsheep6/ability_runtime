...
static std::map<LiveFormExtensionConnectionKey, sptr<JSLiveFormExtensionConnection>, key_compare> g_connects;
...
void RemoveConnection(int64_t connectId)
{
    ...
    if (item != g_connects.end()) {
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "remove conn ability exist");
        if (item->second) {
            item->second->RemoveConnectionObject();
        }
        g_connects.erase(item);
    } else {
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "remove conn ability not exist");
    }
}
 
void FindConnection(AAFwk::Want& want, sptr<JSLiveFormExtensionConnection>& connection, int64_t& connectId)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "Disconnect ability enter, connection:%{public}d.",
    ...
    if (item != g_connects.end()) {
        ...
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "find conn ability exist");
    }
    return;
}
 
bool CheckConnectionParam(napi_env env, napi_value value, sptr<JSLiveFormExtensionConnection>& connection,
    AAFwk::Want& want)
{
    if (!CheckTypeForNapiValue(env, value, napi_object)) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Failed to get connection object");
        return false;
    }
    connection->SetJsConnectionObject(value);
    LiveFormExtensionConnectionKey key;
    ...
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "not find connection, create a new connection");
    return true;
}
...
napi_value JsLiveFormExtensionContext::StartAbility(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsLiveFormExtensionContext, OnStartAbility);
}
 
napi_value JsLiveFormExtensionContext::OpenLink(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsLiveFormExtensionContext, OnOpenLink);
}
 
napi_value JsLiveFormExtensionContext::StartAbilityForResult(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsLiveFormExtensionContext, OnStartAbilityForResult);
}
 
napi_value JsLiveFormExtensionContext::StartAbilityForResultAsCaller(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsLiveFormExtensionContext, OnStartAbilityForResultAsCaller);
}
...
napi_value JsLiveFormExtensionContext::ReportDrawnCompleted(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsLiveFormExtensionContext, OnReportDrawnCompleted);
}
 
napi_value JsLiveFormExtensionContext::OpenAtomicService(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsLiveFormExtensionContext, OnOpenAtomicService);
}
 
napi_value JsLiveFormExtensionContext::OnStartAbility(napi_env env, NapiCallbackInfo& info)
{
    HITRACE_METER_NAME(HITRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "called");
    if (info.argc < ARGC_ONE) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "invalid argc");
    ...
    if (!CheckStartAbilityInputParam(env, info, want, startOptions, unwrapArgc)) {
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "Failed, input param type invalid");
    ...
    NapiAsyncTask::CompleteCallback complete =
        [weak = context_, want, startOptions, unwrapArgc](napi_env env, NapiAsyncTask& task, int32_t status) {
            TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "startAbility begin");
            auto context = weak.lock();
            if (!context) {
                TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "context is released");
    ...
    NapiAsyncTask::ScheduleHighQos("JSLiveFormExtensionContext OnStartAbility",
...
bool JsLiveFormExtensionContext::CreateOpenLinkTask(const napi_env &env, const napi_value &lastParam,
    AAFwk::Want &want, int &requestCode)
{
    ...
    RuntimeTask task = [env, asyncTask](int resultCode, const AAFwk::Want& want, bool isInner) {
        TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "OnOpenLink async callback is begin");
        HandleScope handleScope(env);
        napi_value abilityResult = AppExecFwk::WrapAbilityResult(env, resultCode, want);
        if (abilityResult == nullptr) {
            TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "wrap abilityResult error");
    ...
    if (context == nullptr) {
        TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "context is released");
        return false;
    }
...
void JsLiveFormExtensionContext::RemoveOpenLinkTask(int requestCode)
{
    auto context = context_.lock();
    if (context == nullptr) {
        TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "context is released");
...
napi_value JsLiveFormExtensionContext::OnOpenLink(napi_env env, NapiCallbackInfo& info)
{
    HITRACE_METER_NAME(HITRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "OnOpenLink");
    ...
    if (!ParseOpenLinkParams(env, info, linkValue, openLinkOptions, want)) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "parse openLink arguments failed");
    ...
    if (CheckTypeForNapiValue(env, info.argv[INDEX_TWO], napi_function)) {
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "completionHandler is used");
...
napi_value JsLiveFormExtensionContext::OnOpenLinkInner(napi_env env, const AAFwk::Want& want,
    int requestCode, const std::string& startTime, const std::string& url)
{
    ...
    NapiAsyncTask::CompleteCallback complete = [innerErrorCode, requestCode, startTime, url, this](
        ...
        if (*innerErrorCode == 0) {
            TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "OpenLink succeeded");
            return;
        }
        if (freeInstallObserver_ == nullptr) {
            TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "freeInstallObserver_ is nullptr");
            RemoveOpenLinkTask(requestCode);
            return;
        }
        if (*innerErrorCode == AAFwk::ERR_OPEN_LINK_START_ABILITY_DEFAULT_OK) {
            TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "start ability by default succeeded");
            freeInstallObserver_->OnInstallFinishedByUrl(startTime, url, ERR_OK);
            return;
        }
        TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "OpenLink failed");
    ...
    NapiAsyncTask::ScheduleHighQos("JsLiveFormExtensionContext::OnOpenLink", env,
        CreateAsyncTaskWithLastParam(env, nullptr, std::move(execute), std::move(complete), nullptr));
 
    return result;
}
 
napi_value JsLiveFormExtensionContext::OnStartAbilityForResult(napi_env env, NapiCallbackInfo& info)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "called");
    ...
    if (!CheckStartAbilityInputParam(env, info, want, startOptions, unwrapArgc)) {
        ThrowInvalidParamError(env, "Parse param want failed, want must be Want.");
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "input param type invalid");
        return CreateJsUndefined(env);
    }
    ...
    std::shared_ptr<NapiAsyncTask> asyncTask = std::move(uasyncTask);
    RuntimeTask task = [env, asyncTask](int resultCode, const AAFwk::Want &want, bool isInner) {
        TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "called");
        napi_value abilityResult = AppExecFwk::WrapAbilityResult(env, resultCode, want);
        if (abilityResult == nullptr) {
            TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "wrap abilityResult failed");
            asyncTask->Reject(env, CreateJsError(env, AbilityErrorCode::ERROR_CODE_INNER));
            return;
        }
        if (isInner) {
    ...
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "OnStartAbilityForResult end");
    return result;
}
 
napi_value JsLiveFormExtensionContext::OnStartAbilityForResultAsCaller(napi_env env, NapiCallbackInfo &info)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "called");
    if (info.argc == ARGC_ZERO) {
        ThrowTooFewParametersError(env);
    ...
    auto context = context_.lock();
    if (context == nullptr) {
        TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "The context is released");
        asyncTask->Reject(env, CreateJsError(env, AbilityErrorCode::ERROR_CODE_INVALID_CONTEXT));
        return result;
    }
    want.SetParam(Want::PARAM_RESV_FOR_RESULT, true);
    ...
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "End.");
    return result;
}
...
napi_value JsLiveFormExtensionContext::OnConnectAbility(napi_env env, NapiCallbackInfo& info)
{
    ...
    AAFwk::Want want;
    sptr<JSLiveFormExtensionConnection> connection = new JSLiveFormExtensionConnection(env);
    ...
    NapiAsyncTask::CompleteCallback complete =
        [weak = context_, want, connection, connectId](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "ConnectAbility connection:%{public}d", static_cast<int32_t>(connectId));
    ...
    napi_value result = nullptr;
    NapiAsyncTask::ScheduleHighQos("JSLiveFormExtensionConnection::OnConnectAbility",
...
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "start");
    if (info.argc < ARGC_ONE) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "invalid argc");
        ThrowTooFewParametersError(env);
        return CreateJsUndefined(env);
    ...
    if (!AppExecFwk::UnwrapInt64FromJS2(env, info.argv[INDEX_ZERO], connectId)) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Invalid connectId");
        ThrowInvalidParamError(env, "Parse param connectId failed, connectId must be number");
        return CreateJsUndefined(env);
    }
 
    AAFwk::Want want;
    sptr<JSLiveFormExtensionConnection> connection = nullptr;
    ...
    NapiAsyncTask::CompleteCallback complete =
        [weak = context_, want, connection](
            ...
            if (connection == nullptr) {
                TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "connection nullptr");
                task.Reject(env, CreateJsError(env, AbilityErrorCode::ERROR_CODE_INNER));
                return;
            }
            TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "context->DisconnectAbility");
    ...
    napi_value result = nullptr;
    NapiAsyncTask::Schedule("JSLiveFormExtensionConnection::OnDisconnectAbility",
        env, CreateAsyncTaskWithLastParam(env, lastParam, nullptr, std::move(complete), &result));
    return result;
}
 
napi_value JsLiveFormExtensionContext::OnReportDrawnCompleted(napi_env env, NapiCallbackInfo& info)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "called");
    ...
    napi_value result = nullptr;
    NapiAsyncTask::ScheduleHighQos("JsLiveFormExtensionContext::OnReportDrawnCompleted",
        env, CreateAsyncTaskWithLastParam(env, lastParam, std::move(execute), std::move(complete), &result));
    return result;
}
 
napi_value JsLiveFormExtensionContext::OnOpenAtomicService(napi_env env, NapiCallbackInfo& info)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "start");
    if (info.argc == ARGC_ZERO) {
        ThrowTooFewParametersError(env);
    ...
    if (!ConvertFromJsValue(env, info.argv[INDEX_ZERO], appId)) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "OnOpenAtomicService, parse appId failed");
    ...
    if (info.argc > ARGC_ONE && CheckTypeForNapiValue(env, info.argv[INDEX_ONE], napi_object)) {
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "OnOpenAtomicService atomic service options is used");
        if (!AppExecFwk::UnwrapStartOptionsAndWant(env, info.argv[INDEX_ONE], startOptions, want)) {
            TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Fail to parse atomic service options");
            ThrowInvalidParamError(env,
    ...
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "bundleName: %{public}s", bundleName.c_str());
    want.SetBundle(bundleName);
    return OpenAtomicServiceInner(env, info, want, startOptions, unwrapArgc);
}
 
napi_value JsLiveFormExtensionContext::OpenAtomicServiceInner(napi_env env, NapiCallbackInfo& info, Want &want,
    const AAFwk::StartOptions &options, size_t unwrapArgc)
{
    ...
    RuntimeTask task = [env, element = want.GetElement(), startTime, &observer = freeInstallObserver_](
        ...
        TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "OnOpenAtomicService async callback is begin");
        if (observer == nullptr) {
            TAG_LOGW(AAFwkTag::LIVE_FORM_EXT, "observer is nullptr");
            return;
        }
        HandleScope handleScope(env);
...
void JsLiveFormExtensionContext::AddFreeInstallObserver(napi_env env, const AAFwk::Want &want, napi_value callback,
    napi_value *result, bool isAbilityResult, bool isOpenLink)
{
    // adapter free install async return install and start result
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "begin");
    int ret = 0;
    if (freeInstallObserver_ == nullptr) {
    ...
    if (ret != ERR_OK) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "AddFreeInstallObserver error");
    }
    std::string startTime = want.GetStringParam(Want::PARAM_RESV_START_TIME);
    if (!isOpenLink) {
        TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "AddJsObserverObject");
...
napi_value JsLiveFormExtensionContext::CreateJsLiveFormExtensionContext(napi_env env,
...
bool JsLiveFormExtensionContext::CheckStartAbilityInputParam(napi_env env, NapiCallbackInfo& info,
...
JSLiveFormExtensionConnection::JSLiveFormExtensionConnection(napi_env env) : env_(env) {}
 
JSLiveFormExtensionConnection::~JSLiveFormExtensionConnection()
{
    ReleaseNativeReference(jsConnectionObject_.release());
}
 
void JSLiveFormExtensionConnection::ReleaseNativeReference(NativeReference* ref)
{
    if (ref == nullptr) {
...
void JSLiveFormExtensionConnection::SetConnectionId(int64_t id)
{
    connectionId_ = id;
}
 
int64_t JSLiveFormExtensionConnection::GetConnectionId()
{
    return connectionId_;
}
 
void JSLiveFormExtensionConnection::OnAbilityConnectDone(const AppExecFwk::ElementName &element,
    const sptr<IRemoteObject> &remoteObject, int resultCode)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "resultCode:%{public}d", resultCode);
    wptr<JSLiveFormExtensionConnection> connection = this;
    std::unique_ptr<NapiAsyncTask::CompleteCallback> complete = std::make_unique<NapiAsyncTask::CompleteCallback>
        ([connection, element, remoteObject, resultCode](napi_env env, NapiAsyncTask &task, int32_t status) {
            sptr<JSLiveFormExtensionConnection> connectionSptr = connection.promote();
            if (!connectionSptr) {
                TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "connectionSptr nullptr");
                return;
            }
    ...
    NapiAsyncTask::ScheduleHighQos("JSLiveFormExtensionConnection::OnAbilityConnectDone",
        env_, std::make_unique<NapiAsyncTask>(callback, std::move(execute), std::move(complete)));
}
 
void JSLiveFormExtensionConnection::HandleOnAbilityConnectDone(const AppExecFwk::ElementName &element,
    const sptr<IRemoteObject> &remoteObject, int resultCode)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "start, resultCode:%{public}d", resultCode);
    // wrap ElementName
    ...
    if (jsConnectionObject_ == nullptr) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "jsConnectionObject_ null");
        return;
    }
    napi_value obj = jsConnectionObject_->GetNapiValue();
    if (!CheckTypeForNapiValue(env_, obj, napi_object)) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Failed to get object");
        return;
    }
    napi_value methodOnConnect = nullptr;
    napi_get_named_property(env_, obj, "onConnect", &methodOnConnect);
    if (methodOnConnect == nullptr) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Failed to get onConnect from object");
        return;
    }
    napi_call_function(env_, obj, methodOnConnect, ARGC_TWO, argv, nullptr);
}
 
void JSLiveFormExtensionConnection::OnAbilityDisconnectDone(const AppExecFwk::ElementName &element, int resultCode)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "resultCode:%{public}d", resultCode);
    wptr<JSLiveFormExtensionConnection> connection = this;
    std::unique_ptr<NapiAsyncTask::CompleteCallback> complete = std::make_unique<NapiAsyncTask::CompleteCallback>
        ([connection, element, resultCode](napi_env env, NapiAsyncTask &task, int32_t status) {
            sptr<JSLiveFormExtensionConnection> connectionSptr = connection.promote();
            if (!connectionSptr) {
                TAG_LOGI(AAFwkTag::LIVE_FORM_EXT, "connectionSptr nullptr");
                return;
            }
    ...
    napi_ref callback = nullptr;
    std::unique_ptr<NapiAsyncTask::ExecuteCallback> execute = nullptr;
    NapiAsyncTask::Schedule("JSLiveFormExtensionConnection::OnAbilityDisconnectDone",
        env_, std::make_unique<NapiAsyncTask>(callback, std::move(execute), std::move(complete)));
}
 
void JSLiveFormExtensionConnection::HandleOnAbilityDisconnectDone(const AppExecFwk::ElementName &element,
    int resultCode)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "resultCode:%{public}d", resultCode);
    napi_value napiElementName = OHOS::AppExecFwk::WrapElementName(env_, element);
    ...
    if (jsConnectionObject_ == nullptr) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "jsConnectionObject_ nullptr");
        return;
    }
    napi_value obj = jsConnectionObject_->GetNapiValue();
    if (!CheckTypeForNapiValue(env_, obj, napi_object)) {
    ...
    if (method == nullptr) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Failed to get onDisconnect from object");
        return;
    }
...
void JSLiveFormExtensionConnection::SetJsConnectionObject(napi_value jsConnectionObject)
{
...
void JSLiveFormExtensionConnection::RemoveConnectionObject()
{
    jsConnectionObject_.reset();
}
 
void JSLiveFormExtensionConnection::CallJsFailed(int32_t errorCode)
{
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "CallJsFailed enter");
    if (jsConnectionObject_ == nullptr) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "jsConnectionObject_ nullptr");
        return;
    }
    napi_value obj = jsConnectionObject_->GetNapiValue();
    if (!CheckTypeForNapiValue(env_, obj, napi_object)) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "wrong to get object");
        return;
    }
    ...
    if (method == nullptr) {
        TAG_LOGE(AAFwkTag::LIVE_FORM_EXT, "Failed to get onFailed from object");
        return;
    }
    napi_value argv[] = { CreateJsValue(env_, errorCode) };
    napi_call_function(env_, obj, method, ARGC_ONE, argv, nullptr);
    TAG_LOGD(AAFwkTag::LIVE_FORM_EXT, "CallJsFailed end");
}
 
napi_value JSLiveFormExtensionConnection::CallObjectMethod(const char* name, napi_value const *argv, size_t argc)
{
