...
bool AppRecovery::AddAbility(std::shared_ptr<Ability> ability, const std::shared_ptr<AbilityInfo>& abilityInfo,
    const sptr<IRemoteObject>& token, LaunchReason reason)
{
    if (!isEnable_) {
    ...
    DeleteRecoveryStateFiles(reason);
    return true;
}
...
void AppRecovery::DeleteRecoveryStateFiles(LaunchReason reason)
{
    ...
    HILOG_INFO("AppRecovery DeleteRecoveryStateFiles fileDir: %{public}s, launchReason: %{public}d", fileDir.c_str(), reason);
    ...
    if (reason != LaunchReason::LAUNCHREASON_APP_RECOVERY) {
