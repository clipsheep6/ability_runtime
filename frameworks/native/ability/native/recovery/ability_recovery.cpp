...
bool AbilityRecovery::SaveAbilityState()
{
    ...
    if (saveMode_ == SaveModeFlag::SAVE_WITH_FILE) {
        int32_t saveId = missionId_ > 0 ? missionId_ : abilityInfo->descriptionId;
        HILOG_INFO("AppRecovery SaveAbilityState, saveId:%{public}d,missionId_:%{public}d", saveId, missionId_);
        SerializeDataToFile(saveId, wantParams);
...
bool AbilityRecovery::PersistState()
{
    ...
    if (!params_.IsEmpty()) {
        int32_t saveId = missionId_ > 0 ? missionId_ : abilityInfo->descriptionId;
        SerializeDataToFile(saveId, params_);
    }
    return true;
}
...
bool AbilityRecovery::LoadSavedState(StateReason reason)
{
    ...
    int32_t saveId = missionId_ > 0 ? missionId_ : abilityInfo->descriptionId;
    HILOG_INFO("AppRecovery LoadSavedState, saveId:%{public}d,missionId_:%{public}d", saveId, missionId_);
    if (!ReadSerializeDataFromFile(saveId, params_)) {
        HILOG_ERROR("AppRecovery ScheduleRestoreAbilityState. failed to find record for id:%{public}d", saveId);
        hasLoaded_ = false;
        return hasLoaded_;
    }
