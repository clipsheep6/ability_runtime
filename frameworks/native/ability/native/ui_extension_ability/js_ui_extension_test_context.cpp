...
void JsUIExtensionTestContext::Finalizer(napi_env env, void* data, void* hint)
{
    TAG_LOGD(AAFwkTag::UI_EXT, "JsUIExtensionTestContext Finalizer is called");
    std::unique_ptr<JsUIExtensionTestContext>(static_cast<JsUIExtensionTestContext*>(data));
}

napi_value JsUIExtensionTestContext::WangkailongTestMethod(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_AND_CALL(env, info, JsUIExtensionTestContext, OnWangkailongTestMethod);
}

napi_value JsUIExtensionTestContext::OnWangkailongTestMethod(napi_env env, NapiCallbackInfo& info)
{
    ...
    NapiAsyncTask::ScheduleHighQos("JsUIExtensionTestContext::OnWangkailongTestMethod",
        env, CreateAsyncTaskWithLastParam(env, lastParam, std::move(execute), std::move(complete), &result));
    return result;
}

napi_value JsUIExtensionTestContext::CreateJsUIExtensionTestContext(napi_env env,
    std::shared_ptr<UIExtensionContext> context)
{
    TAG_LOGD(AAFwkTag::UI_EXT, "UIExtensionTestContext begin");
    ...
    std::unique_ptr<JsUIExtensionTestContext> jsContext = std::make_unique<JsUIExtensionTestContext>(context);
    napi_wrap(env, objValue, jsContext.release(), Finalizer, nullptr, nullptr);

    const char *moduleName = "JsUIExtensionTestContext";
    ...
    BindNativeFunction(env, objValue, "wangkailongTestMethod", moduleName, WangkailongTestMethod);
