...
VpnExtension* VpnExtension::Create(const std::unique_ptr<Runtime>& runtime)
{
    if (!runtime) {
    ...
    HILOG_INFO("VpnExtension::Create runtime");
    ...
    const sptr<IRemoteObject> &token)
{
    ...
    HILOG_INFO("VpnExtension begin init context");
}
    ...
    const sptr<IRemoteObject> &token)
{
    ...
    if (context == nullptr) {
        HILOG_ERROR("VpnExtension::CreateAndInitContext context is nullptr");
        return context;
    }
    return context;
}
