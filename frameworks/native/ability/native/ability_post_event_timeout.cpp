...
void AbilityPostEventTimeout::TimingBegin(int64_t delaytime)
{
    ...
    if (handler_ == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITY, "null AbilityPostEventTimeout::TimingBegin %{public}s handler_",
            task_.c_str());
        return;
    }
...
{
    if (handler_ == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITY, "null AbilityPostEventTimeout::TimeEnd %{public}s handler_", task_.c_str());
        return;
    }
...
void AbilityPostEventTimeout::TimeOutProc()
{
    ...
    if (!taskExec_) {
        ...
        TAG_LOGW(AAFwkTag::ABILITY, "exec Failed,Event:%{public}s",
            task_.c_str());
    }
}
}  // namespace AppExecFwk
