...
bool TaskHandlerClient::PostTask(std::function<void()> task, long delayTime)
{
    ...
    if (taskHandler_ == nullptr) {
        if (!CreateRunner()) {
            TAG_LOGE(AAFwkTag::ABILITY, "failed, CreateRunner failed");
            return false;
        }
    }
    ...
    if (!ret) {
        TAG_LOGE(AAFwkTag::ABILITY, "failed, taskHandler_ PostTask failed");
    }
    return ret;
}
...
bool TaskHandlerClient::CreateRunner()
{
    if (taskHandler_ == nullptr) {
        ...
        if (runner == nullptr) {
            TAG_LOGE(AAFwkTag::ABILITY, "failed, runner is nullptr");
            return false;
        }
        taskHandler_ = std::make_shared<TaskHandler>(runner);
        if (taskHandler_ == nullptr) {
            TAG_LOGE(AAFwkTag::ABILITY, "failed, taskHandler_ is nullptr");
            return false;
        }
    }
    return true;
