    ...
    sptr<AAFwk::SessionInfo> sessionInfo)
{
    ...
    TAG_LOGI(AAFwkTag::ABILITY, "sourceState:%{public}d, targetState:%{public}d, "
...
bool PageAbilityImpl::AbilityTransaction(const Want &want, const AAFwk::LifeCycleStateInfo &targetState)
{
    ...
    TAG_LOGD(AAFwkTag::ABILITY, "retVal= %{public}d", static_cast<int>(ret));
    return ret;
}
...
void PageAbilityImpl::DoKeyDown(const std::shared_ptr<MMI::KeyEvent>& keyEvent)
{
    ...
    TAG_LOGI(AAFwkTag::ABILITY, "%{public}s focusState %{public}s", abilityInfo->name.c_str(),
