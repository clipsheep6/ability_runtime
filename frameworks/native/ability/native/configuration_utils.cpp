    ...
    std::shared_ptr<ResourceManager> resourceManager)
{
    ...
    configuration->AddItem(displayId, ConfigurationInner::APPLICATION_DENSITYDPI, density);
    ...
    std::shared_ptr<ResourceManager> resourceManager, bool &configChanged)
{
    ...
    newConfig.AddItem(displayId, ConfigurationInner::APPLICATION_DENSITYDPI, density);
    ...
    HILOG_INFO("Update display config %{public}s for all window.", diffConfiguration->GetName().c_str());
    Rosen::Window::UpdateConfigurationForAll(diffConfiguration);
}

bool ConfigurationUtils::GetDisplayConfig(Rosen::DisplayId displayId, std::string &densityStr,
    std::string &directionStr)
{
    ...
    densityStr = GetDensityStr(density);
    directionStr = GetDirectionStr(height, width);
    HILOG_DEBUG("DisplayId: %{public}" PRIu64", density: %{public}s, direction: %{public}s.", displayId,
        densityStr.c_str(), directionStr.c_str());
    return true;
}

void ConfigurationUtils::UpdateDisplayResConfig(std::shared_ptr<ResourceManager> resourceManager,
    std::string &density, std::string &direction)
{
    // resourceManager has checked in caller function.
    HILOG_INFO("Update resConfig, density: %{public}s, direction: %{public}s.", density.c_str(), direction.c_str());
    std::unique_ptr<Global::Resource::ResConfig> resConfig(Global::Resource::CreateResConfig());
    if (resConfig == nullptr) {
