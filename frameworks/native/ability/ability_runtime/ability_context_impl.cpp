...
ErrCode AbilityContextImpl::StartAbilityAsCaller(const AAFwk::Want &want, int requestCode)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start ability as caller, ret=%{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::StartAbilityWithAccount(const AAFwk::Want& want, int accountId, int requestCode)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start ability with account, ret=%{public}d", err);
    }
    return err;
}
    ...
    int requestCode)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start ability, ret=%{public}d", err);
    }
    return err;
}
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "called, name:%{public}s %{public}s, accountId=%{public}d",
...
ErrCode AbilityContextImpl::StartAbilityForResult(const AAFwk::Want& want, int requestCode, RuntimeTask&& task)
{
    ...
    if (err != ERR_OK && err != AAFwk::START_ABILITY_WAITING) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start ability for result, ret=%{public}d", err);
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "called, accountId:%{private}d", accountId);
    resultCallbacks_.insert(make_pair(requestCode, std::move(task)));
    ErrCode err = AAFwk::AbilityManagerClient::GetInstance()->StartAbility(want, token_, requestCode, accountId);
    if (err != ERR_OK && err != AAFwk::START_ABILITY_WAITING) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start ability for result with account. ret=%{public}d", err);
        OnAbilityResultInner(requestCode, err, want);
    }
    return err;
}
...
ErrCode AbilityContextImpl::StartUIServiceExtensionAbility(const AAFwk::Want& want, int32_t accountId)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start ui service extension is failed, ret: %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::StartServiceExtensionAbility(const AAFwk::Want& want, int32_t accountId)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "start eervice extension ability is failed, red: %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::TerminateAbilityWithResult(const AAFwk::Want& want, int resultCode)
{
    ...
    if (Rosen::SceneBoardJudgement::IsSceneBoardEnabled()) {
        ...
        TAG_LOGI(AAFwkTag::CONTEXT, "sceneboard terminate ability with result, ret=%{public}d", err);
        return static_cast<int32_t>(err);
    } else {
        ErrCode err = AAFwk::AbilityManagerClient::GetInstance()->TerminateAbility(token_, resultCode, &want);
        TAG_LOGI(AAFwkTag::CONTEXT, "ams terminate ability with result, ret=%{public}d", err);
        return err;
    }
#else
    ErrCode err = AAFwk::AbilityManagerClient::GetInstance()->TerminateAbility(token_, resultCode, &want);
    TAG_LOGD(AAFwkTag::CONTEXT, "terminate ability with result.b ret=%{public}d", err);
    return err;
#endif
}
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "called, abilityRecordId: %{public}d", abilityRecordId);
...
ErrCode AbilityContextImpl::ConnectAbility(const AAFwk::Want& want, const sptr<AbilityConnectCallback>& connectCallback)
{
    ...
    if (ret != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "connect ability failed, ret:%{public}d", ret);
    }
    return ret;
}
    ...
    const sptr<AbilityConnectCallback>& connectCallback)
{
    ...
    if (ret != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "connect ability with account failed, ret:%{public}d", ret);
    }
    return ret;
}
...
void AbilityContextImpl::MinimizeAbility(bool fromUser)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "minimize ability is failed %{public}d", err);
    }
}
...
ErrCode AbilityContextImpl::MoveAbilityToBackground()
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "move ability to background failed: %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::MoveUIAbilityToBackground()
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "move ui ability to background failed: %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::TerminateSelf()
{
    ...
    if (Rosen::SceneBoardJudgement::IsSceneBoardEnabled() && sessionToken) {
        TAG_LOGI(AAFwkTag::CONTEXT, "terminate self by SCB");
        AAFwk::Want resultWant;
        sptr<AAFwk::SessionInfo> info = new AAFwk::SessionInfo();
        ...
        if (err != ERR_OK) {
            TAG_LOGE(AAFwkTag::CONTEXT, "terminate self failed %{public}d", err);
        }
        return err;
    }
#else
...
ErrCode AbilityContextImpl::CloseAbility()
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "close ability failed: %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::RequestDialogService(napi_env env, AAFwk::Want &want, RequestDialogResultTask &&task)
{
    ...
    auto resultTask =
        ...
        if (loop == nullptr) {
            TAG_LOGE(AAFwkTag::CONTEXT, "request dialog service, fail to get uv loop");
            return;
        }
    ...
    TAG_LOGD(AAFwkTag::CONTEXT, "request dialog service ret=%{public}d", static_cast<int32_t>(err));
    return err;
}
...
ErrCode AbilityContextImpl::RequestDialogService(AAFwk::Want &want, RequestDialogResultTask &&task)
{
    ...
    auto err = AAFwk::AbilityManagerClient::GetInstance()->RequestDialogService(want, token_);
    TAG_LOGD(AAFwkTag::CONTEXT, "request dialog srvice ret=%{public}d", static_cast<int32_t>(err));
    return err;
}
...
void AbilityContextImpl::RequestDialogResultJSThreadWorker(uv_work_t* work, int status)
{
    TAG_LOGD(AAFwkTag::CONTEXT, "called");
    if (work == nullptr) {
        TAG_LOGE(AAFwkTag::CONTEXT, "request dialog result in js threadWorker, uv_queue_work input work is nullptr");
        return;
    }
    RequestResult* retCB = static_cast<RequestResult*>(work->data);
    if (retCB == nullptr) {
        TAG_LOGE(AAFwkTag::CONTEXT, "request dialog result in js threadWorker, retCB is nullptr");
        delete work;
        work = nullptr;
        return;
    }
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "called, label:%{public}s", label.c_str());
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "ccalledall");
    ...
    AAFwk::WantParams &wantParams, const std::shared_ptr<JsUIExtensionCallback> &uiExtensionCallbacks)
{
    ...
    if (sessionId == 0) {
        TAG_LOGE(AAFwkTag::CONTEXT, "create modal ui extension is failed");
        return ERR_INVALID_VALUE;
    }
...
ErrCode AbilityContextImpl::CreateModalUIExtensionWithApp(const AAFwk::Want &want)
{
    ...
    if (IsUIExtensionExist(want)) {
        TAG_LOGD(AAFwkTag::CONTEXT, "ui extension is exist, not create again");
        return ERR_OK;
    }
    auto abilityCallback = abilityCallback_.lock();
    if (abilityCallback == nullptr) {
...
ErrCode AbilityContextImpl::RequestModalUIExtension(const AAFwk::Want& want)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "request modal ui extension is failed %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::ChangeAbilityVisibility(bool isShow)
{
    ...
    if (err != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "change ability visibility is failed %{public}d", err);
    }
    return err;
}
...
ErrCode AbilityContextImpl::AddFreeInstallObserver(const sptr<IFreeInstallObserver> &observer)
{
    ...
    if (ret != ERR_OK) {
        TAG_LOGE(AAFwkTag::CONTEXT, "add free install observer error, ret: %{public}d", ret);
    }
    return ret;
}
    ...
    RuntimeTask &&task)
{
    ...
    if (err != ERR_OK && err != AAFwk::START_ABILITY_WAITING) {
        TAG_LOGE(AAFwkTag::CONTEXT, "apen atomic service, ret=%{public}d", err);
        OnAbilityResultInner(requestCode, err, want);
    }
    return err;
}
