    ...
    const std::vector<int>& permissionsState, int requestCode, PermissionRequestTask &&task)
{
    ...
    auto resultTask =
        ...
        AbilityContextImpl::ResultCallback* retCB = new AbilityContextImpl::ResultCallback();
...
{
    HILOG_INFO("ResultCallback, uv_queue_work");
    if (work == nullptr) {
        HILOG_ERROR("ResultCallback, uv_queue_work input work is nullptr");
        return;
    }
    AbilityContextImpl::ResultCallback *retCB = (AbilityContextImpl::ResultCallback *)work->data;
    if (retCB == nullptr) {
        HILOG_ERROR("ResultCallback, uv_queue_work retCB is nullptr");
        return;
    }
    ...
    delete work;
    work = nullptr;
    HILOG_INFO("ResultCallback, uv_queue_work. end");
}
