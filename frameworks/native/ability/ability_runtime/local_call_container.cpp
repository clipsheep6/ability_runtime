...
{
    std::lock_guard<std::mutex> lock(mutex_);
    TAG_LOGD(AAFwkTag::LOCAL_CALL, "elementName: %{public}s and id %{public}d",
...
{
    TAG_LOGD(AAFwkTag::LOCAL_CALL,
        "called uri is %{private}s", element.GetURI().c_str());
...
{
    TAG_LOGD(
        AAFwkTag::LOCAL_CALL, "start %{public}s", element.GetURI().c_str());
