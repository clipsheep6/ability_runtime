...
std::string JsModuleReader::GetPresetAppHapPath(const std::string& inputPath) const
{
    ...
    if (moduleName.empty()) {
        HILOG_ERROR("Failed to obtain module name.");
    ...
    if (inputPath.find_first_of("/") == inputPath.find_last_of("/")) {
        ...
        auto getInfoResult = bundleMgrClient->GetBundleInfoForSelf(static_cast<int32_t>(AppExecFwk::GetBundleInfoFlag::
        ...
        if (bundleMgrClient->GetBaseSharedBundleInfos(bundleName_, baseSharedBundleInfos) != 0) {
