...
void JsRuntime::StartDebugMode(bool needBreakPoint, bool isDebug = true)
{
    if (debugMode_) {
...
bool JsRuntime::StartDebugger(bool needBreakPoint, uint32_t instanceId, bool isDebug = true)
{
...
void JsRuntime::StartProfiler(const std::string &perfCmd, bool isDebug = true)
{
    CHECK_POINTER(jsEnv_);
    if (JsRuntime::hasInstance.exchange(true, std::memory_order_relaxed)) {
        instanceId_ = static_cast<uint32_t>(gettid());
