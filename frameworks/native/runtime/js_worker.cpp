...
void CreateContainerScope(int32_t id)
{
#ifdef SUPPORT_GRAPHICS
[[maybe_unused]] ContainerScope* scope = new ContainerScope(id);
#endif
}
void DeleteContainerScope(int32_t id)
{}
}
...
void InitWorkerModule(NativeEngine& engine, const std::string& codePath)
{
    ...
    engine.SetInitContainerScopeFunc(CreateContainerScope);
    engine.SetFinishContainerScopeFunc(DeleteContainerScope);
}
