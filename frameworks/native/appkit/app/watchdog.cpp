...
bool WatchDog::Timer()
{
    ...
    if (cvWatchDog_.wait_for(lck, std::chrono::milliseconds(INI_TIMER_FIRST_SECOND), condition)) {
        HILOG_INFO("cvWatchDog1 is stopped");
        return true;
    }
    while (!stopWatchDog_) {
        if (cvWatchDog_.wait_for(lck, std::chrono::milliseconds(INI_TIMER_SECOND), condition)) {
