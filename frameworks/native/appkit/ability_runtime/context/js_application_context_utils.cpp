...
napi_value JsApplicationContextUtils::GetCurrentTwinAppIndex(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_WITH_NAME_AND_CALL(env, info, JsApplicationContextUtils,
        OnGetCurrentTwinAppIndex, APPLICATION_CONTEXT_NAME);
}

napi_value JsApplicationContextUtils::OnGetCurrentTwinAppIndex(napi_env env, NapiCallbackInfo& info)
{
    ...
    int32_t appIndex_;
    auto ret = context->GetCurrentTwinAppIndex(appIndex_);
...
void JsApplicationContextUtils::BindNativeApplicationContext(napi_env env, napi_value object)
{
    ...
    BindNativeFunction(env, object, "getCurrentTwinAppIndex", MD_NAME,
        JsApplicationContextUtils::getCurrentTwinAppIndex);
