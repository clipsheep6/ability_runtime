...
napi_value JsApplicationContextUtils::GetCurrentAppTwinIndex(napi_env env, napi_callback_info info)
{
    GET_NAPI_INFO_WITH_NAME_AND_CALL(env, info, JsApplicationContextUtils,
        OnGetCurrentAppTwinIndex, APPLICATION_CONTEXT_NAME);
}

napi_value JsApplicationContextUtils::OnGetCurrentAppTwinIndex(napi_env env, NapiCallbackInfo& info)
{
    ...
    int32_t appIndex_;
    auto ret = context->GetCurrentAppTwinIndex(appIndex_);
