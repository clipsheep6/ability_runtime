...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "ApplicationContext RegisterAbilityLifecycleCallback");
...
void ApplicationContext::UnregisterAbilityLifecycleCallback(
    const std::shared_ptr<AbilityLifecycleCallback> &abilityLifecycleCallback)
{
    TAG_LOGD(AAFwkTag::CONTEXT, "ApplicationContext UnregisterAbilityLifecycleCallback");
    std::lock_guard<std::recursive_mutex> lock(callbackLock_);
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "ApplicationContext RegisterEnvironmentCallback");
...
void ApplicationContext::UnregisterEnvironmentCallback(
    const std::shared_ptr<EnvironmentCallback> &environmentCallback)
{
    TAG_LOGD(AAFwkTag::CONTEXT, "ApplicationContext UnregisterEnvironmentCallback");
...
void ApplicationContext::DispatchWindowStageFocus(const std::shared_ptr<NativeReference> &ability,
    const std::shared_ptr<NativeReference> &windowStage)
{
    TAG_LOGD(AAFwkTag::CONTEXT, "%{public}s start.", __func__);
    if (!ability || !windowStage) {
        TAG_LOGE(AAFwkTag::CONTEXT, "ability or windowStage is null");
        return;
    }
    std::lock_guard<std::recursive_mutex> lock(callbackLock_);
    for (auto callback : callbacks_) {
...
void ApplicationContext::NotifyApplicationForeground()
{
    ...
    if (callback == nullptr) {
        TAG_LOGE(AAFwkTag::CONTEXT, "applicationStateCallback is nullptr");
        return;
    }
...
{
    if (contextImpl_ == nullptr) {
        TAG_LOGE(AAFwkTag::CONTEXT, "The contextimpl is nullptr");
        return;
    }
...
int32_t ApplicationContext::RestartApp(const AAFwk::Want& want)
{
    ...
    if (abilityName == "") {
        TAG_LOGE(AAFwkTag::CONTEXT, "abilityName is empty.");
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "colorMode:%{public}d.", colorMode);
    if (colorMode < -1 || colorMode > 1) {
        TAG_LOGE(AAFwkTag::CONTEXT, "colorMode is invalid.");
        return;
    }
...
{
    TAG_LOGD(AAFwkTag::CONTEXT, "language:%{public}s.", language.c_str());
    AppExecFwk::Configuration config;
...
{
    if (contextImpl_ == nullptr) {
        TAG_LOGE(AAFwkTag::CONTEXT, "AbilityContext::contextImpl is nullptr.");
