...
napi_value JsExtensionBaseContext::OnGetApplicationContext(napi_env env, NapiCallbackInfo& info)
{
    ...
    if (applicationContext == nullptr) {
        HILOG_INFO("applicationContext is already released");
        return CreateJsUndefined(env);
    }
...
napi_value CreateJsExtensionBaseContext(napi_env env, napi_value object, std::shared_ptr<ExtensionContext> extensionContext)
{
    ...
    auto jsContext = std::make_unique<JsExtensionBaseContext>(extensionContext);
    SetNamedNativePointer(env, object, EXTENSION_BASE_CONTEXT_NAME, jsContext.release(), JsExtensionBaseContext::Finalizer);
