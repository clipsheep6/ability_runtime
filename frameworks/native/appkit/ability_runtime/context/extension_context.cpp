...
std::string ExtensionContext::GetResourceDir()
{
    ...
    if (hapModuleInfoPtr != nullptr && !hapModuleInfoPtr->moduleName.empty()) {
        std::string dir = std::string(LOCAL_CODE_PATH) + CONTEXT_FILE_SEPARATOR + hapModuleInfoPtr->moduleName + CONTEXT_RESOURCE_END + CONTEXT_FILE_SEPARATOR + UNIQUE_ID;
...
{
    preferencesDir = GetBaseDir() + CONTEXT_FILE_SEPARATOR + CONTEXT_PREFERENCES + CONTEXT_FILE_SEPARATOR + CONTEXT_EXTENSION;
