...
std::string JsAbilityStage::OnStartSpecifiedProcess(const AAFwk::Want &want)
{
    HILOG_DEBUG("JsAbilityStage::OnStartSpecifiedProcess come");
    AbilityStage::OnStartSpecifiedProcess(want);
    ...
    napi_value napiWant = OHOS::AppExecFwk::WrapWant(env, want);
    napi_value methodOnStartSpecifiedProcess = nullptr;
    napi_get_named_property(env, obj, "OnStartSpecifiedProcess", &methodOnStartSpecifiedProcess);
    if (methodOnStartSpecifiedProcess == nullptr) {
        HILOG_ERROR("Failed to get 'OnStartSpecifiedProcess' from AbilityStage object");
        return "";
    }

    napi_value argv[] = { napiWant };
    napi_value flagNative = nullptr;
    napi_call_function(env, obj, methodOnStartSpecifiedProcess, 1, argv, &flagNative);
    return AppExecFwk::UnwrapStringFromJS(env, flagNative);
}
