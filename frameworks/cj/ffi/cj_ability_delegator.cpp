...
std::shared_ptr<AbilityRuntime::ApplicationContext> CJAbilityDelegator::GetAppContext()
{
    ...
    if (context == nullptr) {
        TAG_LOGE(AAFwkTag::DELEGATOR, "failed to get appContext from cj delegator");
        return nullptr;
    }
...
int32_t FFIAbilityDelegatorStartAbility(int64_t id, WantHandle want)
{
    ...
    if (cjDelegator == nullptr) {
        TAG_LOGE(AAFwkTag::DELEGATOR, "cj delegator is null");
        return INVALID_CODE;
    }
...
int32_t FFIGetExitCode(int64_t id)
{
    ...
    if (cjShellCmdResult == nullptr) {
        TAG_LOGE(AAFwkTag::DELEGATOR, "cj shell command result is null");
        return INVALID_CODE;
    }
...
int32_t FFIAbilityDelegatorApplicationContext(int64_t id)
{
    ...
    if (appContext == nullptr) {
        TAG_LOGE(AAFwkTag::DELEGATOR, "app context is null");
        return INVALID_CODE;
    }
