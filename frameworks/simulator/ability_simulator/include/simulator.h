...
public:
    ...
    virtual void TerminateAbility(int64_t abilityId) = 0 const;

    virtual int64_t CreateForm(const std::string& formSrcPath, FormUpdateCallback callback) = 0;
    virtual void RequestUpdateForm(const int64_t formId) = 0 const;
    virtual void DestroyForm(const int64_t formId) = 0 const;
};
} // namespace AbilityRuntime
} // namespace OHOS
