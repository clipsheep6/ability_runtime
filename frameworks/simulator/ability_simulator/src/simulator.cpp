    ...
    if (nativeEngine_) {
        ...
        if (uvLoop != nullptr) {
            ...
            uv_queue_work(uvLoop, &work, [](uv_work_t*) {}, [](uv_work_t *work, int32_t status) {
                TAG_LOGE(AAFwkTag::ABILITY_SIM, "stop uv loop");
                uv_stop(work->loop);
            });
        }
    }
...
napi_value SimulatorImpl::LoadScript(const std::string &srcPath)
{
    ...
    if (objRef->IsNull()) {
        TAG_LOGE(AAFwkTag::ABILITY_SIM, "get export failed");
        return nullptr;
    }
...
bool SimulatorImpl::LoadAbilityStage(uint8_t *buffer, size_t len)
{
    ...
    if (moduleInfo_->srcEntrance.empty()) {
        TAG_LOGD(AAFwkTag::ABILITY_SIM, "empty module src path");
        return true;
    }
    ...
    TAG_LOGD(AAFwkTag::ABILITY_SIM, "moduleSrcPath: %{public}s", moduleSrcPath.c_str());
...
void SimulatorImpl::InitJsAbilityStageContext(napi_value obj)
{
    ...
    if (jsStageContext_ == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITY_SIM, "get LoadSystemModuleByEngine failed");
        return;
    }
...
void SimulatorImpl::InitResourceMgr()
{
    ...
    if (resourceMgr_ == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITY_SIM, "null resourceMgr");
        return;
    }
    ...
    const std::shared_ptr<Rosen::WindowScene> &windowScene)
{
    ...
    if (jsWindowStage == nullptr) {
        TAG_LOGE(AAFwkTag::ABILITY_SIM, "create jsWindowSatge object failed");
        return nullptr;
    }
...
bool SimulatorImpl::OnInit()
{
    ...
    if (globalObj == nullptr) {
        delete nativeEngine;
        TAG_LOGE(AAFwkTag::ABILITY_SIM, "get global object failed");
        return false;
    }
...
std::string SimulatorImpl::GetNativeStrFromJsTaggedObj(napi_value obj, const char* key)
{
    ...
    TAG_LOGD(AAFwkTag::ABILITY_SIM, "success.");
    return ret;
}
