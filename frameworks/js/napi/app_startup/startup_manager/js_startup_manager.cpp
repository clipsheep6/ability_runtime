...
napi_value JsStartupManager::OnGetResult(napi_env env, NapiCallbackInfo &info)
{
    ...
    if (!ConvertFromJsValue(env, info.argv[INDEX_ZERO], startupTask)) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "Parameter error: Failed to convert startupTask,"
            "must be a string.");
        return CreateJsUndefined(env);
    }
    ...
    if (res != ERR_OK || result == nullptr || result->GetResultCode() != ERR_OK) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "%{public}s, failed to get result.");
        return CreateJsUndefined(env);
    }
    if (result->GetResultType() != StartupTaskResult::ResultType::JS) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "%{public}s, the result type is not js.");
        return CreateJsUndefined(env);
    }
    ...
    if (jsResult == nullptr) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "%{public}s, failed to convert to js result.");
        return CreateJsUndefined(env);
    }
...
napi_value JsStartupManager::OnIsInitialized(napi_env env, NapiCallbackInfo &info)
{
    ...
    if (res != ERR_OK) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "%{public}s, failed to get result, res = %{public}d.");
        return CreateJsUndefined(env);
    }
...
int32_t JsStartupManager::GetDependencies(napi_env env, napi_value value, std::vector<std::string> &dependencies)
{
    ...
    if (!isArray) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "Parameter error: StartupTasks must be a Array.");
    ...
    for (size_t i = 0; i < arrayLength; i++) {
        ...
        if (valueType != napi_string) {
            ...
            AbilityRuntimeErrorUtil::ThrowByParamError(env, "Parameter error: "
                "StartupTasks element must be a string.");
            return ERR_ABILITY_RUNTIME_EXTERNAL_INVALID_PARAMETER;
        }
        ...
        if (!ConvertFromJsValue(env, napiDep, startupTask)) {
            TAG_LOGE(AAFwkTag::STARTUP, "convert startupTask name failed.");
            AbilityRuntimeErrorUtil::ThrowByParamError(env, "Parameter error: "
                "Convert startupTask name failed. Please check it.");
            return ERR_ABILITY_RUNTIME_EXTERNAL_INVALID_PARAMETER;
        }
...
int32_t JsStartupManager::GetConfig(napi_env env, napi_value value, std::shared_ptr<StartupConfig> &config)
{
    ...
    if (startupConfig->Init(value) != ERR_OK) {
        ...
        AbilityRuntimeErrorUtil::ThrowByParamError(env, "Parameter error: "
            "Failed to init config, must be a StartupConfig.");
