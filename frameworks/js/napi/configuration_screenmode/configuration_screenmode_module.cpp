...
static napi_value InitScreenModeObject(napi_env env)
{
    napi_value object;
    NAPI_CALL(env, napi_create_object(env, &object));

    NAPI_CALL(env, SetEnumItem(env, object, "IDLE_SCREEN_MODE", -1));
    NAPI_CALL(env, SetEnumItem(env, object, "FULL_SCREEN_MODE", 0));
    NAPI_CALL(env, SetEnumItem(env, object, "HALF_SCREEN_MODE", 1));
 ...
 */
static napi_value ConfigurationScreenModeInit(napi_env env, napi_value exports)
{
    napi_value screenMode = InitScreenModeObject(env);
    NAPI_ASSERT(env, screenMode != nullptr, "failed to create color mode object");

    napi_property_descriptor exportObjs[] = {
        DECLARE_NAPI_PROPERTY("ScreenMode", screenMode),
    };
...
static napi_module _module = {
    ...
    .nm_flags = 0,
    .nm_filename = nullptr,
    .nm_register_func = ConfigurationScreenModeInit,
    .nm_modname = "app.ability.configurationScreenMode",
