...
private:
    napi_value OnEnableAppRecovery(napi_env env, const size_t argc, napi_value* argv)
    {
        ...
        for (size_t i = 0; i < parameterCount; ++i) {
            ...
            napi_typeof(reinterpret_cast<napi_env>(&env), reinterpret_cast<napi_value>(argv[i]), &paramType);
            if (paramType != napi_number) {
                HILOG_ERROR("AppRecoveryApi argv[%{public}s] type isn't number", std::to_string(i).c_str());
                return result;
            }
            int32_t tmp = 0;
            napi_get_value_int32(reinterpret_cast<napi_env>(&env),
                reinterpret_cast<napi_value>(argv[i]), &tmp);
    ...
    napi_value OnSaveAppState(napi_env env, const size_t argc, napi_value* argv)
    {
        ...
        uintptr_t ability = 0;
        if (argc == 1) {
            napi_value value = reinterpret_cast<napi_value>(argv[0]);
