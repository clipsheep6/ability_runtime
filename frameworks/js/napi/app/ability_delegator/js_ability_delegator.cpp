...
napi_value JSAbilityDelegator::OnAddAbilityMonitorSync(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    std::shared_ptr<AbilityMonitor> monitor = nullptr;
    if (!ParseAbilityMonitorPara(env, argc, argv, monitor, true)) {
...
napi_value JSAbilityDelegator::OnAddAbilityStageMonitor(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    bool isExisted = false;
    std::shared_ptr<AbilityStageMonitor> monitor = nullptr;
    if (!ParseAbilityStageMonitorPara(env, argc, argv, monitor, isExisted, false)) {
    ...
    if (!isExisted) {
        AddStageMonitorRecord(env, argv[INDEX_ZERO], monitor);
    }
    return result;
}

napi_value JSAbilityDelegator::OnAddAbilityStageMonitorSync(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    bool isExisted = false;
    std::shared_ptr<AbilityStageMonitor> monitor = nullptr;
    if (!ParseAbilityStageMonitorPara(env, argc, argv, monitor, isExisted, true)) {
...
napi_value JSAbilityDelegator::OnRemoveAbilityMonitor(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
    ...
    if (AbilityDelegatorRegistry::GetAbilityDelegator()) {
        for (auto iter = g_monitorRecord.begin(); iter != g_monitorRecord.end(); ++iter) {
            ...
            bool isEquals = false;
            napi_strict_equals(env, (argv[INDEX_ZERO]), jsMonitor->GetNapiValue(), &isEquals);
            if (isEquals) {
...
napi_value JSAbilityDelegator::OnRemoveAbilityMonitorSync(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
...
napi_value JSAbilityDelegator::OnRemoveAbilityStageMonitor(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
    ...
    if (isExisted) {
        RemoveStageMonitorRecord(env, argv[INDEX_ZERO]);
    }
    return result;
}

napi_value JSAbilityDelegator::OnRemoveAbilityStageMonitorSync(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
...
napi_value JSAbilityDelegator::OnWaitAbilityMonitor(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
    ...
    int64_t timeout = 0;
    if (!ParseWaitAbilityMonitorPara(env, argc, argv, monitor, opt, timeout)) {
    ...
    if (opt.hasCallbackPara) {
        lastParam = opt.hasTimeoutPara ? argv[INDEX_TWO] : argv[INDEX_ONE];
    }
...
napi_value JSAbilityDelegator::OnWaitAbilityStageMonitor(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
    ...
    TimeoutCallback opt {false, false};
    int64_t timeout = 0;
    if (!ParseWaitAbilityStageMonitorPara(env, argc, argv, monitor, opt, timeout)) {
...
napi_value JSAbilityDelegator::OnPrint(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    std::string msg;
    if (!ParsePrintPara(env, argc, argv, msg)) {
...
napi_value JSAbilityDelegator::OnPrintSync(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
...
napi_value JSAbilityDelegator::OnExecuteShellCommand(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
    ...
    if (!ParseExecuteShellCommandPara(env, argc, argv, cmd, opt, timeoutSecs)) {
...
napi_value JSAbilityDelegator::OnGetAppContext(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
...
napi_value JSAbilityDelegator::OnGetAbilityState(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
    ...
    if (!ParseAbilityPara(env, argv[INDEX_ZERO], remoteObject)) {
...
napi_value JSAbilityDelegator::OnGetCurrentTopAbility(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
...
napi_value JSAbilityDelegator::OnStartAbility(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    AAFwk::Want want;
    if (!ParseStartAbilityPara(env, argc, argv, want)) {
...
napi_value JSAbilityDelegator::OnDoAbilityForeground(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    sptr<OHOS::IRemoteObject> remoteObject = nullptr;
    if (!ParseAbilityCommonPara(env, argc, argv, remoteObject)) {
...
napi_value JSAbilityDelegator::OnDoAbilityBackground(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));
...
napi_value JSAbilityDelegator::OnFinishTest(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    std::string msg;
    int64_t code = 0;
    if (!ParseFinishTestPara(env, argc, argv, msg, code)) {
...
napi_value JSAbilityDelegator::OnSetMockList(napi_env env, size_t argc, napi_value* argv)
{
    HILOG_INFO("enter, argc = %{public}d", static_cast<int32_t>(argc));

    std::map<std::string, std::string> mockList;
    if (!ParseMockListPara(env, argc, argv, mockList)) {
...
napi_value JSAbilityDelegator::ParseAbilityMonitorPara(
    napi_env env, size_t argc, napi_value* argv, std::shared_ptr<AbilityMonitor> &monitor, bool isSync)
{
    HILOG_INFO("enter");
    if (argc < ARGC_ONE) {
        HILOG_ERROR("Incorrect number of parameters");
    ...
    if (!ParseMonitorPara(env, argv[INDEX_ZERO], monitor)) {
    ...
    if (argc > ARGC_ONE) {
        if (!AppExecFwk::IsTypeForNapiValue(env, argv[INDEX_ONE], napi_function)) {
...
napi_value JSAbilityDelegator::ParseAbilityStageMonitorPara(napi_env env, size_t argc, napi_value* argv,
    std::shared_ptr<AbilityStageMonitor> &monitor, bool &isExisted, bool isSync)
{
    ...
    if (!ParseStageMonitorPara(env, argv[INDEX_ZERO], monitor, isExisted)) {
...
napi_value JSAbilityDelegator::ParseWaitAbilityMonitorPara(napi_env env, size_t argc, napi_value* argv,
    std::shared_ptr<AbilityMonitor> &monitor, TimeoutCallback &opt, int64_t &timeout)
{
    ...
    if (!ParseTimeoutCallbackPara(env, argc, argv, opt, timeout)) {
...
napi_value JSAbilityDelegator::ParseWaitAbilityStageMonitorPara(napi_env env, size_t argc, napi_value* argv,
...
napi_value JSAbilityDelegator::ParseTimeoutCallbackPara(
    napi_env env, size_t argc, napi_value* argv, TimeoutCallback &opt, int64_t &timeout)
{
    HILOG_INFO("enter");
    ...
    if (argc >= ARGC_TWO) {
        if (ConvertFromJsValue(env, argv[INDEX_ONE], timeout)) {
            opt.hasTimeoutPara = true;
        } else {
            if (argv[INDEX_ONE] == nullptr) {
                HILOG_WARN("argv[1] is null");
            } else if (AppExecFwk::IsTypeForNapiValue(env, argv[INDEX_ONE], napi_function)) {
        ...
        if (argc > ARGC_TWO) {
            if (!AppExecFwk::IsTypeForNapiValue(env, argv[INDEX_TWO], napi_function)) {
                if (argv[INDEX_TWO] == nullptr) {
...
napi_value JSAbilityDelegator::ParsePrintPara(napi_env env, size_t argc, napi_value* argv, std::string &msg)
{
    HILOG_INFO("enter");
    if (argc < ARGC_ONE) {
        HILOG_ERROR("Incorrect number of parameters");
    ...
    if (!ConvertFromJsValue(env, argv[INDEX_ZERO], msg)) {
...
napi_value JSAbilityDelegator::ParseExecuteShellCommandPara(
    napi_env env, size_t argc, napi_value* argv, std::string &cmd, TimeoutCallback &opt, int64_t &timeout)
{
    HILOG_INFO("enter");
    if (argc < ARGC_ONE) {
        HILOG_ERROR("Incorrect number of parameters");
    ...
    if (!ConvertFromJsValue(env, argv[INDEX_ZERO], cmd)) {
...
napi_value JSAbilityDelegator::ParseAbilityCommonPara(
    napi_env env, size_t argc, napi_value* argv, sptr<OHOS::IRemoteObject> &remoteObject)
{
    HILOG_INFO("enter");
    if (argc < ARGC_ONE) {
        HILOG_ERROR("Incorrect number of parameters");
...
napi_value JSAbilityDelegator::ParseStartAbilityPara(
    napi_env env, size_t argc, napi_value* argv, AAFwk::Want &want)
{
    HILOG_INFO("enter");
    if (argc < ARGC_ONE) {
        HILOG_ERROR("Incorrect number of parameters");
    ...
    if (!OHOS::AppExecFwk::UnwrapWant(
        reinterpret_cast<napi_env>(&env), reinterpret_cast<napi_value>(argv[INDEX_ZERO]), want)) {
...
napi_value JSAbilityDelegator::ParseFinishTestPara(
    napi_env env, size_t argc, napi_value* argv, std::string &msg, int64_t &code)
{
    HILOG_INFO("enter");
    if (argc < ARGC_TWO) {
        HILOG_ERROR("Incorrect number of parameters");
    ...
    if (!ConvertFromJsValue(env, argv[INDEX_ONE], code)) {
...
bool JSAbilityDelegator::ParseMockListPara(
    napi_env env, size_t argc, napi_value* argv, std::map<std::string, std::string> &mockList)
{
    HILOG_DEBUG("enter");
    if (argc != ARGC_ONE) {
        HILOG_ERROR("Incorrect number of parameters");
