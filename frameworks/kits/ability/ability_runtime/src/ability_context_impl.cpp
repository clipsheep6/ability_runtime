...
{
    HILOG_DEBUG("AbilityContextImpl::StartAbilityWithAccount. Start calling ams->StartAbility.");
    ErrCode err = AAFwk::AbilityManagerClient::GetInstance()->StartAbility(want, token_, requestCode, accountId);
    HILOG_INFO("AbilityContextImpl::StartAbilityWithAccount. End calling ams->StartAbility. ret=%{public}d", err);
    return err;
}
...
{
    HILOG_DEBUG("%{public}s. Start calling ams->StartAbilityForResultWithAccount. accountId:%{public}d",
        __func__, accountId);
    resultCallbacks_.insert(make_pair(requestCode, std::move(task)));
    ErrCode err = AAFwk::AbilityManagerClient::GetInstance()->StartAbility(want, token_, requestCode, accountId);
    HILOG_INFO("%{public}s. End calling ams->StartAbilityForResultWithAccount. ret=%{public}d", __func__, err);
    return err;
}
