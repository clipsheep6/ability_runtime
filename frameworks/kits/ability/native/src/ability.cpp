...
{
    BYTRACE_NAME(BYTRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    HILOG_INFO("%{public}s begin restart=%{public}s,startId=%{public}d.", __func__, restart ? "true" : "false", startId);
    if (abilityLifecycleExecutor_ == nullptr) {
...
void Ability::HandleCreateAsContinuation(const Want &want)
{
    ...
    if (success && reversible) {
        ...
        if (reverseContinuationSchedulerReplica_ == nullptr) {
            HILOG_ERROR("Ability::HandleCreateAsContinuation failed, create reverseContinuationSchedulerReplica failed");
            return;
        }
    }
...
void Ability::HandleFormMessage(const int32_t msgCode, const FormJsInfo &formJsInfo)
{
    ...
    std::shared_ptr<FormCallback> formCallback = nullptr;
    {
        ...
        // call the callback function when you need to be notified
        if (appCallbackIterator == appCallbacks_.end()) {
            HILOG_ERROR("%{public}s failed, callback not find, formId: %{public}" PRId64 ".", __func__, formJsInfo.formId);
            return;
        }
        formCallback = appCallbackIterator->second;
    }
...
bool Ability::LifecycleUpdate(std::vector<int64_t> formIds, int32_t updateType)
{
    ...
    if (result != ERR_OK) {
        HILOG_ERROR("%{public}s error, internal error.", __func__);
        return false;
    }
...
bool Ability::RequestForm(const int64_t formId, const Want &want)
{
    ...
    if (resultCode != ERR_OK) {
        HILOG_ERROR("%{public}s error, failed to notify the form service that the form user's lifecycle is updated, error "
                 "code is %{public}d.",
            __func__,
            resultCode);
        return false;
...
bool Ability::GetAllFormsInfo(std::vector<FormInfo> &formInfos)
{
    ...
    if (resultCode != ERR_OK) {
        HILOG_ERROR("%{public}s error, failed to GetAllFormsInfo, error code is %{public}d.", __func__, resultCode);
        return false;
    }
...
bool Ability::GetFormsInfoByModule(std::string &bundleName, std::string &moduleName, std::vector<FormInfo> &formInfos)
{
    ...
    if (bundleName.empty() || moduleName.empty()) {
        HILOG_WARN("Failed to Get forms info, because empty bundleName or moduleName");
        return false;
    }
    ...
    if (resultCode != ERR_OK) {
        HILOG_ERROR("%{public}s error, failed to GetFormsInfoByModule, error code is %{public}d.", __func__, resultCode);
        return false;
    }
    ...
    int numRefs)
{
    ...
    for (auto iterMap : predicatesBackReferencesMap) {
        HILOG_INFO("Ability::ParsePredictionArgsReference predicatesBackReferencesMap first:%{public}d second:%{public}d",
            iterMap.first,
            iterMap.second);
