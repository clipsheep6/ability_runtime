...
void ContinuationManager::ContinueAbility(bool reversible, const std::string &deviceId)
{
    ...
    if (progressState_ != ProgressState::INITIAL) {
        HILOG_ERROR("ContinuationManager::ContinueAbility failed. Another request in progress. progressState_: %{public}d",
            progressState_);
        return;
    }

    if (continuationState_ != ContinuationState::LOCAL_RUNNING) {
        HILOG_ERROR("ContinuationManager::ContinueAbility failed. Illegal continuation state. Current state is %{public}d",
            continuationState_);
        return;
    }
...
bool ContinuationManager::ReverseContinueAbility()
{
    HILOG_INFO("%{public}s called begin", __func__);
    if (progressState_ != ProgressState::INITIAL) {
        HILOG_ERROR("ContinuationManager::ReverseContinueAbility failed. progressState_ is %{public}d.", progressState_);
        return false;
    }
...
void ContinuationManager::CompleteContinuation(int result)
{
    HILOG_INFO("%{public}s called begin", __func__);
    if (CheckContinuationIllegal()) {
        HILOG_ERROR("ContinuationManager::CompleteContinuation failed. Ability not available to complete continuation.");
        return;
    }
