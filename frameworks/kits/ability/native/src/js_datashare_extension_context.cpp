...
public:
    ...
    static void Finalizer(NativeEngine* engine, void* data, void* hint)
    {
        ...
        std::unique_ptr<JsDataShareExtensionContext>(static_cast<JsDataShareExtensionContext*>(data));
    }

    static NativeValue* StartAbility(NativeEngine* engine, NativeCallbackInfo* info)
    {
        JsDataShareExtensionContext* me = CheckParamsAndGetThis<JsDataShareExtensionContext>(engine, info);
...
private:
    std::weak_ptr<DataShareExtensionContext> context_;
    ...
    NativeValue* OnConnectAbility(NativeEngine& engine, NativeCallbackInfo& info)
    {
        ...
        HILOG_INFO("%{public}s bundlename:%{public}s abilityname:%{public}s",
            ...
            want.GetElement().GetAbilityName().c_str());
        // unwarp connection
        sptr<JSDataShareExtensionConnection> connection = new JSDataShareExtensionConnection();
    ...
    NativeValue* OnDisconnectAbility(NativeEngine& engine, NativeCallbackInfo& info)
    {
        ...
        // unwrap connectId
        int64_t connectId = -1;
        sptr<JSDataShareExtensionConnection> connection = nullptr;
        ...
        auto item = std::find_if(connects_.begin(), connects_.end(),
            [&connectId](
                const std::map<ConnecttionKey, sptr<JSDataShareExtensionConnection>>::value_type &obj) {
...
NativeValue* CreateJsDataShareExtensionContext(NativeEngine& engine, std::shared_ptr<DataShareExtensionContext> context)
{
    HILOG_INFO("CreateJsDataShareExtensionContext begin");
    NativeValue* objValue = CreateJsExtensionContext(engine, context);
    NativeObject* object = ConvertNativeValueTo<NativeObject>(objValue);

    std::unique_ptr<JsDataShareExtensionContext> jsContext = std::make_unique<JsDataShareExtensionContext>(context);
    object->SetNativePointer(jsContext.release(), JsDataShareExtensionContext::Finalizer, nullptr);

    // make handler
    handler_ = std::make_shared<AppExecFwk::EventHandler>(AppExecFwk::EventRunner::GetMainEventRunner());

    BindNativeFunction(engine, *object, "startAbility", JsDataShareExtensionContext::StartAbility);
    BindNativeFunction(engine, *object, "terminateSelf", JsDataShareExtensionContext::TerminateAbility);
    BindNativeFunction(engine, *object, "connectAbility", JsDataShareExtensionContext::ConnectAbility);
    BindNativeFunction(engine, *object, "disconnectAbility", JsDataShareExtensionContext::DisconnectAbility);

    return objValue;
}

void JSDataShareExtensionConnection::OnAbilityConnectDone(const AppExecFwk::ElementName &element,
    const sptr<IRemoteObject> &remoteObject, int resultCode)
{
    ...
    wptr<JSDataShareExtensionConnection> connection = this;
    auto task = [connection, element, remoteObject, resultCode]() {
        sptr<JSDataShareExtensionConnection> connectionSptr = connection.promote();
        if (!connectionSptr) {
...
void JSDataShareExtensionConnection::HandleOnAbilityConnectDone(const AppExecFwk::ElementName &element,
    const sptr<IRemoteObject> &remoteObject, int resultCode)
{
    ...
    HILOG_INFO("JSDataShareExtensionConnection::CallFunction onConnect, success");
    // two params
    if (engine_ == nullptr) {
...
void JSDataShareExtensionConnection::OnAbilityDisconnectDone(const AppExecFwk::ElementName &element, int resultCode)
{
...
void JSDataShareExtensionConnection::HandleOnAbilityDisconnectDone(const AppExecFwk::ElementName &element,
    int resultCode)
{
...
void JSDataShareExtensionConnection::SetNativeEngine(NativeEngine* engine)
{
    engine_ = engine;
}

void JSDataShareExtensionConnection::SetJsConnectionObject(NativeValue* jsConnectionObject)
{
    jsConnectionObject_ = std::unique_ptr<NativeReference>(engine_->CreateReference(jsConnectionObject, 1));
}

void JSDataShareExtensionConnection::CallJsFailed(int32_t errorCode)
{
