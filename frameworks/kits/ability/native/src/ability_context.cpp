...
ErrCode AbilityContext::TerminateAbility()
{
    ...
    switch (info->type) {
        case AppExecFwk::AbilityType::PAGE:
            HILOG_ERROR("%{public}s begin ams->TerminateAbility for PAGE.", __func__);
            if (resultWant_.HasParameter("ohos.user.grant.permission")) {
                HILOG_ERROR("AbilityContext::TerminateAbility, TerminateAbility execute contained permission key.");
            }
