...
#define PAC_MAP_PUT_FLOAT_VALUE(id, iid, key, value, mapList) \
    RemoveData(mapList, key);                                 \
    sptr<iid> val = id::Box(value);                           \
    mapList.emplace(key, val);
...
#define GET_BASE_FlOAT_DATA_VALUE(iid, it, value, json, type) \
    iid *data = iid::Query(it->second.GetRefPtr());           \
    value val = 0;                                            \
    data->GetValue(val);                                      \
    char buffer[BUFFER_LENGTH] = {0};                         \
    int ret = sprintf_s(buffer, sizeof(buffer), "%.6f", val); \
    if (ret == -1) {                                          \
        return false;                                         \
    }                                                         \
    json["data"] = std::string(buffer);                       \
    json["type"] = type;
