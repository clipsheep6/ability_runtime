...
bool WantParams::ReadFromParcelLong(Parcel &parcel, const std::string &key)
{
    ...
    if (parcel.ReadInt64(value)) {
        ...
        if (intf) {
            SetParam(key, intf);
            APP_LOGI("%{public}s insert param: key=%{public}s, value=%{public}s", __func__, 
...
bool WantParams::ReadFromParcelParam(Parcel &parcel, const std::string &key, int type)
{
    switch (type) {
        ...
        case VALUE_TYPE_STRING:
            APP_LOGI("%{public}s type=VALUE_TYPE_STRING, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelString(parcel, key);
        case VALUE_TYPE_BOOLEAN:
            APP_LOGI("%{public}s type=VALUE_TYPE_BOOLEAN, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelBool(parcel, key);
        case VALUE_TYPE_BYTE:
            APP_LOGI("%{public}s type=VALUE_TYPE_BYTE, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelInt8(parcel, key);
        case VALUE_TYPE_CHAR:
            APP_LOGI("%{public}s type=VALUE_TYPE_CHAR, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelChar(parcel, key);
        case VALUE_TYPE_SHORT:
            APP_LOGI("%{public}s type=VALUE_TYPE_SHORT, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelShort(parcel, key);
        case VALUE_TYPE_INT:
            APP_LOGI("%{public}s type=VALUE_TYPE_INT, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelInt(parcel, key);
        case VALUE_TYPE_LONG:
            APP_LOGI("%{public}s type=VALUE_TYPE_LONG, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelLong(parcel, key);
        case VALUE_TYPE_FLOAT:
            APP_LOGI("%{public}s type=VALUE_TYPE_FLOAT, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelFloat(parcel, key);
        case VALUE_TYPE_DOUBLE:
            APP_LOGI("%{public}s type=VALUE_TYPE_DOUBLE, key=%{public}s.", __func__, key.c_str());
            return ReadFromParcelDouble(parcel, key);
        case VALUE_TYPE_NULL:
            APP_LOGI("%{public}s type=VALUE_TYPE_NULL, key=%{public}s.", __func__, key.c_str());
            break;
        case VALUE_TYPE_PARCELABLE:
        case VALUE_TYPE_PARCELABLEARRAY:
        case VALUE_TYPE_SERIALIZABLE:
        case VALUE_TYPE_LIST:
            APP_LOGI("%{public}s type=VALUE_TYPE_PARCELABLE|VALUE_TYPE_PARCELABLEARRAY|VALUE_TYPE_SERIALIZABLE|"
                "VALUE_TYPE_LIST, key=%{public}s.", __func__, key.c_str());
        ...
        default: {
            // handle array
            APP_LOGI("%{public}s type=XXXXX_ARRAY_XXXX, key=%{public}s.", __func__, key.c_str());
            sptr<IArray> ao = nullptr;
            if (!ReadArrayToParcel(parcel, type, ao)) {
                APP_LOGI("%{public}s read array fail.", __func__);
                return false;
            }
            sptr<IInterface> intf = ao;
            if (intf) {
                SetParam(key, intf);
                APP_LOGI("%{public}s save data to param. key=%{public}s", __func__, key.c_str());
            } else {
                APP_LOGI("%{public}s save data to param fail, intf=nullptr. key=%{public}s", 
                    __func__, key.c_str());
            }
            break;
        }
