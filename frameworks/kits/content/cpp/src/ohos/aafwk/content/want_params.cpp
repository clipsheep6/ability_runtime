...
{
    HILOG_INFO("yangliang before call NewParams: size: %{public}u, source: %{public}p, dest: %{public}p", source.params_.size(), &source, &dest);
    // Deep copy
    for (auto it = source.params_.begin(); it != source.params_.end(); it++) {
        ...
        HILOG_INFO("yangliang after call NewParams: source: %{public}p, dest: %{public}p, %{public}u", &source, &dest, dest.params_.size());
    }
    return true;
}  // namespace AAFwk
// inner use
