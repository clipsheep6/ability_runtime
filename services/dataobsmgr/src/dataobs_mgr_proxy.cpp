...
int32_t DataObsManagerProxy::RegisterObserver(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    ...
    auto error = SendRequest(IDataObsMgr::REGISTER_OBSERVER, data, reply, option);
    if (error != NO_ERROR) {
...
int32_t DataObsManagerProxy::UnregisterObserver(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    ...
    auto error = SendRequest(IDataObsMgr::UNREGISTER_OBSERVER, data, reply, option);
    if (error != NO_ERROR) {
        HILOG_ERROR("failed, SendRequest error: %{public}d, uri: %{public}s", error,
            CommonUtils::Anonymous(uri.ToString()).c_str());
        return error;
...
int32_t DataObsManagerProxy::NotifyChange(const Uri &uri)
{
    ...
    auto error = SendRequest(IDataObsMgr::NOTIFY_CHANGE, data, reply, option);
    if (error != NO_ERROR) {
        HILOG_ERROR("failed, SendRequest error: %{public}d, uri: %{public}s", error,
            CommonUtils::Anonymous(uri.ToString()).c_str());
        return IPC_ERROR;
    ...
    bool isDescendants)
{
    ...
    auto error = SendRequest(IDataObsMgr::REGISTER_OBSERVER_EXT, data, reply, option);
    if (error != NO_ERROR) {
...
Status DataObsManagerProxy::UnregisterObserverExt(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    ...
    auto error = SendRequest(IDataObsMgr::UNREGISTER_OBSERVER_EXT, data, reply, option);
    if (error != NO_ERROR) {
        HILOG_ERROR("failed, SendRequest error: %{public}d, uri: %{public}s", error,
            CommonUtils::Anonymous(uri.ToString()).c_str());
        return IPC_ERROR;
...
Status DataObsManagerProxy::UnregisterObserverExt(sptr<IDataAbilityObserver> dataObserver)
{
    ...
    auto error = SendRequest(IDataObsMgr::UNREGISTER_OBSERVER_ALL_EXT, data, reply, option);
    if (error != NO_ERROR) {
...
Status DataObsManagerProxy::NotifyChangeExt(const ChangeInfo &changeInfo)
{
    ...
    auto error = SendRequest(IDataObsMgr::NOTIFY_CHANGE_EXT, data, reply, option);
    if (error != NO_ERROR) {
...
int32_t DataObsManagerProxy::SendRequest(uint32_t code, MessageParcel &data,
