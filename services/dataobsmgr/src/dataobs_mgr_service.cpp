      ...
      state_(DataObsServiceRunningState::STATE_NOT_START)
{
    ...
    dataObsMgrInnerRdb_ = std::make_shared<DataObsMgrInnerRdb>();
}
...
int DataObsMgrService::RegisterObserver(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    ...
    int status;
    if (const_cast<Uri &>(uri).GetScheme() == SHARED_RDB) {
      status = dataObsMgrInnerRdb_->HandleRegisterObserver(uri, dataObserver);
    } else {
...
int DataObsMgrService::UnregisterObserver(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    ...
    int status;
    if (const_cast<Uri &>(uri).GetScheme() == SHARED_RDB) {
      status = dataObsMgrInnerRdb_->HandleUnregisterObserver(uri, dataObserver);
    } else {
...
int DataObsMgrService::NotifyChange(const Uri &uri)
{
    ...
    if (dataObsMgrInner_ == nullptr || dataObsMgrInnerExt_ == nullptr || dataObsMgrInnerRdb_ == nullptr) {
        HILOG_ERROR("dataObsMgrInner_ is nullptr, uri:%{public}s", CommonUtils::Anonymous(uri.ToString()).c_str());
        return DATAOBS_SERVICE_INNER_IS_NULL;
    }
    ...
    handler_->SubmitTask([this, uri, changeInfo]() {
        if (const_cast<Uri &>(uri).GetScheme() == SHARED_RDB) {
            dataObsMgrInnerRdb_->HandleNotifyChange(uri);
        } else {
