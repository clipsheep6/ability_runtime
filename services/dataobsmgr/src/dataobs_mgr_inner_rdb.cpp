...
DataObsMgrInnerRdb::DataObsMgrInnerRdb() {}

DataObsMgrInnerRdb::~DataObsMgrInnerRdb() {}

int DataObsMgrInnerRdb::HandleRegisterObserver(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    std::lock_guard<ffrt::mutex> lock(innerRdbMutex_);
...
int DataObsMgrInnerRdb::HandleUnregisterObserver(const Uri &uri, sptr<IDataAbilityObserver> dataObserver)
{
    std::lock_guard<ffrt::mutex> lock(innerRdbMutex_);
...
int DataObsMgrInnerRdb::HandleNotifyChange(const Uri &uri)
{
...
void DataObsMgrInnerRdb::AddObsDeathRecipient(sptr<IDataAbilityObserver> dataObserver)
{
    ...
    if (it != obsRecipient_.end()) {
        ...
        std::weak_ptr<DataObsMgrInnerRdb> thisWeakPtr(shared_from_this());
...
void DataObsMgrInnerRdb::RemoveObsDeathRecipient(sptr<IRemoteObject> dataObserver)
{
...
void DataObsMgrInnerRdb::OnCallBackDied(const wptr<IRemoteObject> &remote)
{
...
void DataObsMgrInnerRdb::RemoveObs(sptr<IRemoteObject> dataObserver)
{
...
bool DataObsMgrInnerRdb::HaveRegistered(sptr<IDataAbilityObserver> dataObserver)
{
