...
{
    GTEST_LOG_(INFO) << "AbilityManagerServiceAccountTest SetUp called";
...
{
    GTEST_LOG_(INFO) << "AbilityManagerServiceAccountTest TearDown called";
    abilityMs_->OnStop();
...
HWTEST_F(AbilityManagerServiceAccountTest, Account_ScheduleDisconnectAbilityDone_001, TestSize.Level1)
{
    ...
    WaitUntilTaskFinished();
    EXPECT_EQ(result3, ERR_INVALID_VALUE);
 ...
 * EnvConditions: NA
 * CaseDescription: Verify function AcquireDataAbility
 */
HWTEST_F(AbilityManagerServiceAccountTest, Account_AcquireDataAbility_001, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "AbilityManagerServiceAccountTest Account_AcquireDataAbility_001 start";
    abilityMs_->StartUser(USER_ID_U100);
    ...
    GTEST_LOG_(INFO) << "AbilityManagerServiceAccountTest Account_AcquireDataAbility_001 end";
}
...
HWTEST_F(AbilityManagerServiceAccountTest, Account_TerminateAbilityResult_001, TestSize.Level1)
{
    ...
    if (serviceMapIter != abilityMs_->GetConnectManagerByUserId(USER_ID_U100)->serviceMap_.end()) {
        ...
        GTEST_LOG_(INFO) << "Not found";
    }
    GTEST_LOG_(INFO) << "start id "<< serviceMapIter->second->GetStartId();
