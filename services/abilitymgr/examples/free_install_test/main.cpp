...
int main()
{
    std::unique_ptr<AbilityManagerClient> client_;
    client_.reset(new (std::nothrow) AbilityManagerClient());
    auto ret = client_->Connect();
    if (ret != 0) {
    ...
    client_->StartAbility(want, nullptr);
    return 0;
}
