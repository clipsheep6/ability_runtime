...
{
    return std::string(TASK_DATA_FILE_BASE_PATH) + "/" + std::to_string(userId_) + "/" + std::string(MISSION_DATA_FILE_PATH);
}
...
bool MissionDataStorage::CheckFileNameValid(const std::string &fileName)
{
    ...
    if (fileNameExcludePath.find(JSON_FILE_SUFFIX) != fileNameExcludePath.length() - std::string(JSON_FILE_SUFFIX).length()) {
        return false;
    }
