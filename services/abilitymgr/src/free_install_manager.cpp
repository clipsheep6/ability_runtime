    ...
    const sptr<IRemoteObject> &callerToken, bool isAsync)
{
    ...
    CHECK_POINTER_AND_RETURN(client, GET_BUNDLE_MANAGER_SERVICE_FAILED);
    ...
    if (IN_PROCESS_CALL(client->QueryAbilityInfo(info.want, flag, info.userId, abilityInfo, callback))) {
    ...
    const sptr<IRemoteObject> &callerToken, const std::string& localDeviceId)
{
    ...
    if (!isSaCall) {
        ...
        auto res = IN_PROCESS_CALL(client->GetNameForUid(callerUid, localBundleName));
    ...
    if (!IN_PROCESS_CALL(client->QueryAbilityInfo(
            want, AppExecFwk::AbilityInfoFlag::GET_ABILITY_INFO_WITH_APPLICATION, userId, abilityInfo)) &&
        !IN_PROCESS_CALL(client->QueryExtensionAbilityInfos(
    ...
    if (resultCode == ERR_OK) {
        auto updateAtmoicServiceTask = [want, userId, thisWptr, &timeStampMap = timeStampMap_]() {
            ...
            if (timeStampMap.find(nameKey) == timeStampMap.end() ||
                ...
                client->UpgradeAtomicService(want, userId);
