    ...
    const sptr<IRemoteObject> &callerToken, bool isAsync)
{
    ...
    if (IN_PROCESS_CALL(bundleMgrClient->QueryAbilityInfo(info.want, flag, info.userId, abilityInfo, callback))) {
    ...
    const sptr<IRemoteObject> &callerToken, const std::string& localDeviceId)
{
    ...
    if (!isSaCall) {
        ...
        auto res = IN_PROCESS_CALL(bundleMgrClient->GetNameForUid(callerUid, localBundleName));
    ...
    if (!IN_PROCESS_CALL(bundleMgrClient->QueryAbilityInfo(
            want, AppExecFwk::AbilityInfoFlag::GET_ABILITY_INFO_WITH_APPLICATION, userId, abilityInfo)) &&
        !IN_PROCESS_CALL(bundleMgrClient->QueryExtensionAbilityInfos(
    ...
    if (resultCode == ERR_OK) {
        auto updateAtmoicServiceTask = [want, userId, thisWptr, &timeStampMap = timeStampMap_]() {
            ...
            if (timeStampMap.find(nameKey) == timeStampMap.end() ||
                ...
                bundleMgrClient->UpgradeAtomicService(want, userId);
