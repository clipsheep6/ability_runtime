    ...
    if (StartAbilityUtils::GetCallerAbilityInfo(param.callerToken, callerAbilityInfo)) {
        ...
        if (!targetAbilityInfo.applicationInfo.isSystemApp &&
            !DelayedSingleton<ExtensionConfig>::GetInstance()->IsExtensionStartThirdPartyAppEnable(
                callerAbilityInfo.extensionTypeName, param.want.GetElement().GetBundleName())) {
            TAG_LOGE(AAFwkTag::ABILITYMGR, "The extension start has been blocked by third party app list.");
            return EXTENSION_BLOCKED_BY_THIRD_PARTY_APP_LIST;
        }
