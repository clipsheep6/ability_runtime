...
void AbilityRecord::SaveResultToCallers(const int resultCode, const Want *resultWant)
{
    ...
    for (auto caller : callerRecordList) {
        ...
        if (caller != lastestCaller) {
            HILOG_WARN("Caller record is not the latest.");
...
void SystemAbilityCallerRecord::SetResultToSystemAbility(std::shared_ptr<SystemAbilityCallerRecord> callerSystemAbilityRecord,
