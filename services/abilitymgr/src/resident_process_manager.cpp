...
void ResidentProcessManager::StartupResidentProcess(const std::vector<AppExecFwk::BundleInfo> &bundleInfos)
{
    DelayedSingleton<AppScheduler>::GetInstance()->StartupResidentProcess(bundleInfos);
}

void ResidentProcessManager::StartMainElement(std::vector<AppExecFwk::BundleInfo> &bundleInfos)
{
    ...
    for (size_t i = 0; i < bundleInfos.size(); i++) {
        ...
        for (auto hapModuleInfo : bundleInfos[i].hapModuleInfos) {
            ...
            DelayedSingleton<AbilityManagerService>::GetInstance()->StartAbility(want, USER_ID_NO_HEAD, DEFAULT_INVAL_VALUE);
        }
    }
