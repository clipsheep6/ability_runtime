...
ErrCode AbilityManagerClient::AbilityWindowConfigTransitionDone(sptr<IRemoteObject> token, int state, const WindowConfig &saveData)
{
    auto abms = GetAbilityManager();
    CHECK_POINTER_RETURN_NOT_CONNECTED(abms);
    return abms->AbilityWindowConfigTransitionDone(token, state, saveData);
}
