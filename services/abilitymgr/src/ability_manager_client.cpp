...
ErrCode AbilityManagerClient::UpdateConfiguration(const GlobalConfiguration &config, std::string changeType)
{
    CHECK_REMOTE_OBJECT_AND_RETURN(remoteObject_, ABILITY_SERVICE_NOT_CONNECTED);
    sptr<IAbilityManager> abms = iface_cast<IAbilityManager>(remoteObject_);
    return abms->UpdateConfiguration(config, changeType);
}
