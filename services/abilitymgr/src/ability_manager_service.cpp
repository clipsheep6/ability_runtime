    ...
    const Want &want, const sptr<IAbilityConnection> &connect, const sptr<IRemoteObject> &callerToken, int32_t userId)
{
    ...
    if (CheckIsFreeInstall(want)) {
        ...
        std::string deviceId = want.GetElement().GetDeviceID();
        std::string moduleName = want.GetStringParam("moduleName");
        HILOG_INFO("AbilityManagerService::ConnectAbility. moduleName is %{public}s", moduleName.c_str());
        std::string localDeviceId;
        ...
        if (bundleName.empty() || abilityName.empty() || moduleName.empty()) {
            HILOG_ERROR("AbilityManagerService::ConnectAbility. bundleName or abilityName or moduleName is empty");
            return ERR_INVALID_VALUE;
        }
        int callerUid = IPCSkeleton::GetCallingUid();
        std::string bundleNameLocal;
        iBundleManager_->GetBundleNameForUid(callerUid, bundleNameLocal);
        if (bundleNameLocal != bundleName) {
            HILOG_ERROR("AbilityManagerService::ConnectAbility. bundleName is not local bundleName");
            return ERR_INVALID_VALUE;
        }
        ...
        if (!(iBundleManager_->QueryAbilityInfo(
            ...
            int result = StartFreeInstall(want, validUserId, DEFAULT_INVAL_VALUE);
            if (result) {
