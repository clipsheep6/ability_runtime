...
void AbilityManagerService::EnableRecoverAbility(const sptr<IRemoteObject>& token)
{
    ...
    if (record->IsTerminating()) {
        HILOG_ERROR("AppRecovery ability is terminating, not enabled");
        return;
    }

    InnerMissionInfo innerMissionInfo;
    auto missionId = record->GetMissionId();
    auto missionInfoMgr = DelayedSingleton<MissionInfoMgr>::GetInstance();
    if (missionInfoMgr->GetInnerMissionInfoById(missionId, innerMissionInfo) != 0) {
        HILOG_ERROR("AppRecovery Failed to find mission info for %{public}d", missionId);
        return;
    }

    innerMissionInfo.missionInfo.hasRecoverInfo = true;
    if (!missionInfoMgr->UpdateMissionInfo(innerMissionInfo)) {
        HILOG_ERROR("AppRecovery Failed to update mission info for %{public}d", missionId);
        return;
    }
    HILOG_INFO("AppRecovery hasRecoverableState missionId:%{public}d, recoverable:%{public}d", missionId,
        innerMissionInfo.missionInfo.hasRecoverInfo);
