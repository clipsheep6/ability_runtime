...
void AbilityManagerService::StartResidentApps()
{
    ...
    HILOG_INFO("StartupResidentProcess GetBundleInfos size: %{public}lu", (unsigned long)bundleInfos.size());

    DelayedSingleton<ResidentProcessManager>::GetInstance()->StartMainElement(bundleInfos);
    if (!bundleInfos.empty()) {
        DelayedSingleton<ResidentProcessManager>::GetInstance()->StartupResidentProcess(bundleInfos);
    }
}
