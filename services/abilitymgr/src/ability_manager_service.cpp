...
int AbilityManagerService::PrepareTerminateAbility(const sptr<IRemoteObject> &token, int resultCode,
    const Want *resultWant)
{
    HILOG_DEBUG("Prepare terminate ability begin.");
    if (!prepareTerminateEnableFlag_) {
        HILOG_ERROR("Get prepare terminate parameter false or value is false.");
