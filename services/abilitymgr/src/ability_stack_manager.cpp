    ...
    if (isMissionChanged) {
        if (isStackChanged) {
            if (isCurrentFull && isTargetFull) {
                ...
                auto IsChanged = [&lastTopAbility, &targetAbility, stackManager = shared_from_this()]() {
                    bool isSameMission = lastTopAbility->GetMissionRecordId() == targetAbility->GetMissionRecordId();
                    bool isSyncVisual = stackManager->SupportSyncVisualByStackId(targetAbility->GetMissionStackId());
...
#define TERMINATE(ability)                                                    \
    ability->SetTerminatingState();                                           \
    auto ret = TerminateAbilityLocked(ability, DEFAULT_INVAL_VALUE, nullptr); \
    if (ret != ERR_OK) {                                                      \
    ...
    for (auto &ability : abilityInfos) {
        ...
        TERMINATE(abilityRecord);
    }

    if (activeAbility) {
        TERMINATE(activeAbility);
    }
...
{
    //HILOG_DEBUG("Request mission option size :%{public}d", missionOptions.size());
    ...
    const std::shared_ptr<AbilityRecord> &currentTopAbility, const AbilityRequest &abilityRequest) const
{
    ...
    // if (ERR_OK != DelayedSingleton<AbilityManagerService>::GetInstance()->CheckPermission(
    //                   abilityRequest.abilityInfo.bundleName, AbilityConfig::FLOATING_WINDOW_PERMISSION)) {
    //     HILOG_ERROR("Request ability not system float window permission.");
...
std::shared_ptr<MissionStack> AbilityStackManager::GetOrCreateMissionStack(int stackId, bool isCreateFlag)
{
    ...
    if (isCreateFlag) {
        ...
        //HILOG_DEBUG("Add stackId(%{public}d) to missionStackList, mission stack size:%{public}d",
        //    stackId,
