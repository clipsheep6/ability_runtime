...
int AbilityManagerProxy::FreeInstallAbility(const Want &want, const sptr<IRemoteObject> &callerToken,
    const sptr<IRemoteObject> &statusCallback, int32_t userId, int requestCode)
{
    ...
    if (!data.WriteParcelable(statusCallback)) {
        HILOG_ERROR("statusCallback write failed.");
        return INNER_ERR;
    }
    ...
    auto error = Remote()->SendRequest(IAbilityManager::FREE_INSTALL_ABILITY, data, reply, option);
    if (error != NO_ERROR) {
        HILOG_ERROR("Send request error: %{public}d", error);
        return error;
    }
