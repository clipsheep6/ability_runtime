...
{
    if (!callback) {
        HILOG_ERROR("%{public}s: callback is nullptr.", __func__);
        return INNER_ERR;
    }
    int error;
    MessageParcel data;
    ...
    if (!data.WriteRemoteObject(callback->AsObject())) {
        HILOG_ERROR("%{public}s: callback write failed.", __func__);
        return INNER_ERR;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
    ...
    if (callback == nullptr) {
        HILOG_INFO("luc001.1',callback==nullptr.");
    } else {
        HILOG_INFO("luc001.1',callback!=nullptr.");
    }
    // sptr<IRemoteObject> handler(reply.ReadRemoteObject());
    // callback->AsObject = handler;
    return reply.ReadInt32();
}
#endif
