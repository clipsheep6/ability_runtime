    ...
    std::shared_ptr<StartOptions> &startOptions, int32_t missionId)
{
    ...
    if (isCold) {
        auto task = [self, targetAbilityRecord, startOptions, want] {
            auto mgr = self.lock();
            if (mgr) {
                mgr->StartingWindowCode(targetAbilityRecord, startOptions, want);
            }
        };
        ...
        auto task = [self, targetAbilityRecord, startOptions, want, missionId] {
            auto mgr = self.lock();
            if (mgr) {
                mgr->StartingWindowHot(targetAbilityRecord, startOptions, want, missionId);
            }
        };
        handler->PostTask(task, 0);
    }
    ...
    const std::shared_ptr<AbilityRecord> &callerAbility)
{
    ...
    if (isCold) {
        auto task = [self, targetAbilityRecord, abilityRequest] {
            auto mgr = self.lock();
            if (mgr) {
                mgr->StartingWindowCode(abilityRequest, targetAbilityRecord);
            }
        };
        handler->PostTask(task, 0);
    } else {
        ...
        auto task = [self, targetAbilityRecord, abilityRequest, missionId] {
            auto mgr = self.lock();
            if (mgr) {
                mgr->StartingWindowHot(abilityRequest, targetAbilityRecord, missionId);
            }
        };
        handler->PostTask(task, 0);
    }
...
void MissionListManager::ProcessTimeOut(int64_t eventId) const
{
    std::lock_guard<std::recursive_mutex> guard(managerLock_);
    auto abilityRecord = GetAbilityRecordByEventId(eventId);
    if (abilityRecord == nullptr) {
