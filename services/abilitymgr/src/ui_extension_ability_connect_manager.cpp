...
UIExtensionAbilityConnectManager::UIExtensionAbilityConnectManager(int userId) : userId_(userId)
{
...
int32_t UIExtensionAbilityConnectManager::GenerateUIExtensionAbilityId(int32_t uiExtensionAbilityId)
{
...
void UIExtensionAbilityConnectManager::AddUIExtensionAbilityRecord(int32_t uiExtensionAbilityId,
...
void UIExtensionAbilityConnectManager::RemoveUIExtensionAbilityRecord(int32_t uiExtensionAbilityId)
{
    HILOG_DEBUG("UIExtensionAbilityId %{public}d.", uiExtensionAbilityId);
    std::lock_guard<std::mutex> lock(mutex_);
...
bool UIExtensionAbilityConnectManager::CheckUIExtensionAbilityLoaded(int32_t uiExtensionAbilityId,
    std::string hostBundleName)
{
    ...
    if (it != uiExtensionAbilityRecords_.end() && it->second != nullptr) {
        ...
        HILOG_DEBUG("Stored host bundleName: %{public}s, input bundleName is %{public}s.",
            it->second->hostBundleName.c_str(), hostBundleName.c_str());
        if (it->second->hostBundleName == hostBundleName) {
            return true;
        }
    }
...
bool UIExtensionAbilityConnectManager::CheckUIExtensionAbilityTerminable(int32_t uiExtensionAbilityId)
{
    HILOG_DEBUG("UIExtensionAbilityId %{public}d.", uiExtensionAbilityId);
    std::lock_guard<std::mutex> lock(mutex_);
    // find target record firstly
    ...
    if (it != uiExtensionAbilityRecords_.end() && it->second != nullptr) {
        // check connection
        auto abilityRecord = it->second->abilityRecord_;
        if (abilityRecord != nullptr && abilityRecord->IsConnectListEmpty()) {
            return true;
        }
    }
    return false;
