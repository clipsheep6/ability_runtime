...
const std::string EXTENSION_CONFIG_NAME = "ams_extension_config";
...
const std::string EXTENSION_THIRD_PARTY_APP_BLOCKED_LIST_NAME = "third_party_app_blocked_list";
const std::string EXTENSION_SERVICE_BLOCKED_LIST_NAME = "service_blocked_list";
...
bool ExtensionConfig::IsExtensionStartThirdPartyAppEnable(std::string extensionTypeName, std::string targetBundleName)
{
    if (thirdPartyAppBlockedLists_.find(extensionTypeName) != thirdPartyAppBlockedLists_.end() &&
        thirdPartyAppBlockedLists_[extensionTypeName].find(targetBundleName) !=
        thirdPartyAppBlockedLists_[extensionTypeName].end()) {
        return false;
    }
    return true;
}
...
bool ExtensionConfig::IsExtensionStartServiceEnable(std::string extensionTypeName, std::string targetUri)
{
    ...
    for (auto iter : serviceBlockedLists_[extensionTypeName]) {
...
{
    if (!object.contains(EXTENSION_THIRD_PARTY_APP_BLOCKED_LIST_NAME) ||
        !object.at(EXTENSION_THIRD_PARTY_APP_BLOCKED_LIST_NAME).is_array()) {
        TAG_LOGE(AAFwkTag::ABILITYMGR, "Third party config not existed.");
        return;
    }
    std::unordered_set<std::string> thirdPartyAppBlockedList;
    for (auto &item : object.at(EXTENSION_THIRD_PARTY_APP_BLOCKED_LIST_NAME).items()) {
        const nlohmann::json& jsonObject = item.value();
        if (!jsonObject.is_string()) {
            continue;
        }
        thirdPartyAppBlockedList.emplace(jsonObject.get<std::string>());
    }
    thirdPartyAppBlockedLists_[extensionTypeName] = thirdPartyAppBlockedList;
    TAG_LOGD(AAFwkTag::ABILITYMGR, "The size of %{public}s extension's third party app blocked list is %{public}zu",
        extensionTypeName.c_str(), thirdPartyAppBlockedList.size());
}
...
bool ExtensionConfig::CheckServiceExtensionUriValid(std::string uri)
{
