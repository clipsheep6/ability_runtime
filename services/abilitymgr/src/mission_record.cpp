...
void MissionRecord::SetMissionOption(const MissionOption &option)
{
    if (option.winModeKey != option_.winModeKey) {
        ...
        for (auto &it : abilities_) {
            CHECK_POINTER(it);
            bool flag = option_.winModeKey != AbilityWindowConfiguration::MULTI_WINDOW_DISPLAY_UNDEFINED &&
                        option_.winModeKey != AbilityWindowConfiguration::MULTI_WINDOW_DISPLAY_FULLSCREEN;
            // true : old is multi win, target is fullscreen.
            // false : old is fullscreen, target is multi win.
            auto key = flag ? option_.winModeKey : option.winModeKey;
            if (it->IsReady()) {
