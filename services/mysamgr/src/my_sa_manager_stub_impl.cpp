...
void MySaManagerStubImpl::GrantUriPermission(const Uri &uri, unsigned int flag,
    const Security::AccessToken::AccessTokenID fromTokenId, const Security::AccessToken::AccessTokenID targetTokenId)
{
    ...
    if ((flag & (Want::FLAG_AUTH_READ_URI_PERMISSION | Want::FLAG_AUTH_WRITE_URI_PERMISSION)) == 0) {
        HILOG_WARN("MySaManagerStubImpl::GrantUriPermission: The param flag is invalid.");
        return;
    }
...
bool MySaManagerStubImpl::VerifyUriPermission(const Uri &uri, unsigned int flag,
    const Security::AccessToken::AccessTokenID tokenId)
{
    if ((flag & (Want::FLAG_AUTH_READ_URI_PERMISSION | Want::FLAG_AUTH_WRITE_URI_PERMISSION)) == 0) {
        HILOG_WARN("MySaManagerStubImpl:::VerifyUriPermission: The param flag is invalid.");
        return false;
    }
...
void MySaManagerStubImpl::RemoveUriPermission(const Security::AccessToken::AccessTokenID tokenId)
{
    std::lock_guard<std::mutex> guard(mutex_);
...
sptr<AppExecFwk::IBundleMgr> MySaManagerStubImpl::ConnectBundleManager()
{
...
void MySaManagerStubImpl::ClearProxy()
{
    HILOG_DEBUG("%{public}s is called.", __func__);
    std::lock_guard<std::mutex> lock(bmsMutex_);
...
void MySaManagerStubImpl::BMSDeathRecipient::OnRemoteDied([[maybe_unused]] const wptr<IRemoteObject>& remote)
{
    if (proxy_) {
...
int MySaManagerStubImpl::GetCurrentAccountId()
{
