...
void AppMgrServiceInner::SendHiSysEvent(const int32_t innerEventId, const int64_t eventId)
{
    ...
    APP_LOGD("SendHiSysEvent , eventName=%{public}s, uidStr=%{public}s, pidStr=%{public}s, packageName=%{public}s,\
        processName=%{public}s, msg=%{public}s", eventName.c_str(), uidStr.c_str(), pidStr.c_str(),
        packageName.c_str(), processName.c_str(), msg.c_str());
