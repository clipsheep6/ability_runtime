...
void AppMgrServiceInner::ClearAppRunningDataForKeepAlive(const std::shared_ptr<AppRunningRecord> &appRecord)
{
    ...
    if (appRecord->IsKeepAliveApp()) {
        if (ExitResidentProcessInfo::GetInstance().RecordExitResidentBundleName(appRecord->GetBundleName())) {
...
int32_t AppMgrServiceInner::NotifyMemonySizeStateChanged(bool isMemorySizeSufficent)
{
    ...
    auto StartExitKeepAliveProcessTask = [exitBundleNames, innerServicerWeak = weak_from_this()]() {
        ...
        innerServicer->StartResidentProcessWithMainElement(exitBundleInfos);
        if (!exitBundleInfos.empty()) {
            innerServicer->LoadResidentProcess(exitBundleInfos);
        }
    };
...
void AppMgrServiceInner::StartResidentProcessWithMainElement(const std::vector<AppExecFwk::BundleInfo> &bundleInfos)
{
    std::lock_guard lock(appStateCallbacksLock_);
    for (const auto &callback : appStateCallbacks_) {
        if (callback != nullptr) {
