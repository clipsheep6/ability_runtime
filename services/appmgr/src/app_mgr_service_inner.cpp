...
void AppMgrServiceInner::AddWatchParameter()
{
    ...
    ret = WatchParameter(std::string(AAFwk::GlobalConfigurationKey::INPUT_POINTER_DEVICE), PointerDeviceEventCallback,
        context);
    if (ret != 0) {
        HILOG_ERROR("watch parameter %{public}s failed with %{public}d.",
            std::string(AAFwk::GlobalConfigurationKey::INPUT_POINTER_DEVICE), ret);
    }
}
...
void AppMgrServiceInner::PointerDeviceEventCallback(const char *key, const char *value, void *context)
{
    ...
    if ((strcmp(key, std::string(AAFwk::GlobalConfigurationKey::INPUT_POINTER_DEVICE)) != 0) ||
