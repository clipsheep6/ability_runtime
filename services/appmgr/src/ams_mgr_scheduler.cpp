...
void AmsMgrScheduler::TerminateAbility(const sptr<IRemoteObject> &token, bool isClearMissionFlag)
{
    if (!IsReady()) {
        return;
    }
    std::function<void()> terminateAbilityFunc =
        std::bind(&AppMgrServiceInner::TerminateAbility, amsMgrServiceInner_, token, isClearMissionFlag);
