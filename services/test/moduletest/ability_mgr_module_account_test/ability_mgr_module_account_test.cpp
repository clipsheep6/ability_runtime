...
HWTEST_F(AbilityMgrModuleAccountTest, AbilityMgrModuleAccountTest_StartAbility_007, TestSize.Level1)
{
    ...
    PacMap saveData;
    abilityMgrServ_->AbilityTransitionDone(topAbility->GetToken(), AbilityLifeCycleState::ABILITY_STATE_FOREGROUND_NEW, saveData);
...
HWTEST_F(AbilityMgrModuleAccountTest, AbilityMgrModuleAccountTest_ConnectAbility_004, TestSize.Level1)
{
    ...
    std::shared_ptr<AbilityRecord> record = abilityMgrServ_->GetConnectManagerByUserId(MOCK_MAIN_USER_ID)->GetServiceRecordByToken(testToken);
    EXPECT_TRUE(record);
    ElementName element;
