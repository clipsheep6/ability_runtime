...
{
    const char* tagNames[] = { "AAFwk", "Ability", "Test", "AATool", "AbilitySimulator" };
...
{
    const char* tagNames[] = { "AppDfr", "AppMgr", "DbObsMgr", "Dialog", "Quickfix",
        "UriPermMgr", "BundleMgr", "AppKit" };
...
inline const char* GetDomainName3(AAFwkLogTag tag)
{
    const char* tagNames[] = { "Delegator", "Context", "UIAbility", "Want", "Mission",
        "Connection", "AbilityMgr", "EcologicalRule", "DataAbility" };
...
#define AAFWK_PRINT_LOG(level, tag, fmt, ...)                                                           \
    do {                                                                                                \
        ...
        OHOS::AAFwk::GetTagInfoFromDomainId(logTag), fmt, ##__VA_ARGS__)); \
    } while (0)
