...
void BundleManagerUtils::QueryAbilityInfo(const Want &want, int32_t appIndex, int32_t flags, int32_t userId,
    AppExecFwk::AbilityInfo &abilityInfo)
{
    HILOG_INFO("QueryAbilityInfo come, call bms GetSandboxAbilityInfo, userId is %{public}d", userId);
    IN_PROCESS_CALL_WITHOUT_RET(bundleClient_.GetSandboxAbilityInfo(want, appIndex, flags, userId, abilityInfo));
}

void BundleManagerUtils::QueryExtensionAbilityInfos(const Want &want, int32_t appIndex, const int32_t &flags,
    const int32_t &userId, std::vector<AppExecFwk::ExtensionAbilityInfo> &extensionInfos)
{
    HILOG_INFO("QueryExtensionAbilityInfos come, call bms GetSandboxExtAbilityInfos, userId is %{public}d", userId);
    IN_PROCESS_CALL_WITHOUT_RET(bundleClient_.GetSandboxExtAbilityInfos(want, appIndex, flags, userId, extensionInfos));
}
}  // namespace AAFwk
}  // namespace OHOS
