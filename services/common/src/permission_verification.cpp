...
int PermissionVerification::CheckCallDataAbilityPermission(const VerificationInfo &verificationInfo)
{
    ...
    if (verificationInfo.apiTargetVersion > API8 &&
        !JudgeStartAbilityFromBackground(verificationInfo.accessTokenId, verificationInfo.isBackgroundCall)) {
        return CHECK_PERMISSION_FAILED;
    }
...
int PermissionVerification::CheckStartByCallPermission(const VerificationInfo &verificationInfo)
{
    ...
    if (!JudgeStartAbilityFromBackground(verificationInfo.accessTokenId, verificationInfo.isBackgroundCall)) {
        return CHECK_PERMISSION_FAILED;
    }
...
bool PermissionVerification::JudgeStartAbilityFromBackground(const uint32_t accessTokenId, const bool isBackgroundCall)
{
    if (!isBackgroundCall) {
