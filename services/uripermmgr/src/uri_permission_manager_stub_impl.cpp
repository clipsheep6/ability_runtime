    ...
    uint32_t targetTokenId)
{
    ...
    std::vector<std::string> removedUriList;
    {
        ...
        for (auto &uri : uriVec) {
            if (!CheckUriTypeIsValid(uri)) {
                HILOG_DEBUG("only support file uri.");
                invalidUriCount++;
                continue;
            }
            auto search = uriMap_.find(uri.ToString());
            if (search == uriMap_.end()) {
                HILOG_DEBUG("URI does not exist on uri map, uri is %{public}s", uri.ToString().c_str());
                continue;
            }
            ...
            for (auto it = recordList.begin(); it != recordList.end(); it++) {
                ...
                if (permission && it->targetTokenId == targetTokenId) {
                    HILOG_DEBUG("Found a valid uri permission record, uri is %{public}s", uri.ToString().c_str());
