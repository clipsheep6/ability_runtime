...
#undef RTLIB_NAME
    ...
    if (!LoadSymbolInitUIScheduler(dso, lazyApis_)) {
        return false;
    }
    if (!LoadSymbolRunUIScheduler(dso, lazyApis_)) {
        return false;
    }
    if (!LoadSymbolFiniCJRuntime(dso, lazyApis_)) {
        return false;
    }
    if (!LoadSymbolInitCJLibrary(dso, lazyApis_)) {
        return false;
    }
    if (!LoadSymbolRegisterEventHandlerCallbacks(dso, lazyApis_)) {
        return false;
    }
#ifdef __OHOS__
