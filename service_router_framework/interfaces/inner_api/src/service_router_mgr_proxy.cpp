...
int32_t ServiceRouterMgrProxy::SendRequest(uint32_t code, MessageParcel &data,
                                           MessageParcel &reply, MessageOption &option)
{
    APP_LOGI("ServiceRouterMgrProxy SendRequest");
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
        APP_LOGE("remote object is nullptr.");
        return ERR_APPEXECFWK_FAILED_GET_REMOTE_PROXY;
    }
    int32_t result = remote->SendRequest(code, data, reply, option);
    if (result != OHOS::NO_ERROR) {
        APP_LOGE("SendRequest failed. code is %{public}d, ret is %{public}d.", code, result);
    }
    return result;
}
