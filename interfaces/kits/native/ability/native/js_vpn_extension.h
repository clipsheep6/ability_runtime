...
public:
    ...
    /**
     * @brief Called back when Vpn is started.
     * This method can be called only by Vpn. You can use the StartAbility(ohos.aafwk.content.Want) method to start
     * Vpn. Then the system calls back the current method to use the transferred want parameter to execute its own
     * logic.
     *
