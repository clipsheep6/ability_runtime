...
#ifndef OHOS_ABILITY_RUNTIME_JS_LIVE_FORM_EXTENSION_CONTEXT_H
#define OHOS_ABILITY_RUNTIME_JS_LIVE_FORM_EXTENSION_CONTEXT_H
...
class JsLiveFormExtensionContext final {
public:
    explicit JsLiveFormExtensionContext(const std::shared_ptr<LiveFormExtensionContext> &context) : context_(context) {}
    ...
    static napi_value CreateJsLiveFormExtensionContext(napi_env env, std::shared_ptr<LiveFormExtensionContext> context);
...
class JSLiveFormExtensionConnection : public AbilityConnectCallback {
public:
    explicit JSLiveFormExtensionConnection(napi_env env);
    ~JSLiveFormExtensionConnection();
...
struct LiveFormExtensionConnectionKey {
    AAFwk::Want want;
    int64_t id;
};
 
struct key_compare {
    bool operator()(const LiveFormExtensionConnectionKey &key1, const LiveFormExtensionConnectionKey &key2) const
    {
        if (key1.id < key2.id) {
            return true;
        }
