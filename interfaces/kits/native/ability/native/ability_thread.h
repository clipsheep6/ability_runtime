...
public:
     ...
     * AbilitySlice#onAbilityResult(int, int, Want) method to receive the result
     * set in the current method. This method can be called only after the ability
     ...
     * @param resultData Indicates the data returned after the ability is
     * destroyed. You can define the data returned. This parameter can be null.
     */
     ...
     * sub-sections of files that often assets inside of their .hap.
     * @param uri Indicates the path of the file to open.
     * @param mode Indicates the file open mode, which can be "r" for read-only
     * access, "w" for write-only access (erasing whatever data is currently in
     * the file), "wt" for write access that truncates any existing file, "wa" for
