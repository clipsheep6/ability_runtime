...
napi_value GetContinueAbilityOptionsReversible(const napi_env &env, const napi_value &value, ContinueAbilityOptionsInfo &info)
{
    HILOG_INFO("%{public}s.", __func__);
...
napi_value GetContinueAbilityOptionsDeviceID(const napi_env &env, const napi_value &value, ContinueAbilityOptionsInfo &info)
{
    HILOG_INFO("%{public}s.", __func__);
    napi_valuetype valuetype = napi_undefined;
    napi_value result = nullptr;
...
void ConnectAbilityCallbackCompletedCB(napi_env env, napi_status status, void *data)
{
    ...
    napi_value callResult = 0;
    napi_get_undefined(env, &undefined);
    HILOG_INFO("%{public}s errCode=%{public}d result=%{public}d id=%{public}lld.",
        __func__,
...
napi_value ConnectAbilityWrap(napi_env env, napi_callback_info info, ConnectAbilityCB *connectAbilityCB)
{
    ...
    HILOG_INFO("%{public}s id:%{public}lld.", __func__, connectAbilityCB->id);
...
void NAPIAbilityConnection::OnAbilityDisconnectDone(const AppExecFwk::ElementName &element, int resultCode)
{
    ...
    uv_queue_work(
        ...
        [](uv_work_t *work, int status) {
            ...
            // release connect
            HILOG_INFO("OnAbilityDisconnectDone connects_.size:%{public}d", connects_.size());
            ...
            if (item != connects_.end()) {
                ...
                HILOG_INFO("OnAbilityDisconnectDone erase connects_.size:%{public}d", connects_.size());
            }
