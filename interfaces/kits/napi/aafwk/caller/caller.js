...
class Caller {
    ...
    getProxy() {
        console.log("Caller getProxy " + typeof this.__call_obj__);
        return this;
    }

    testCall(method, data) {
        return new Promise(async (resolve, reject) => {
            console.log("Caller testCall method + [" + method + "]");
            if (typeof method !== 'string' || typeof data !== 'object') {
                console.log("Caller testCall " + typeof method + " " + typeof data);
                reject(new Error("function input parameter error"));
                return;
            }

            if (method == '' || data == null) {
                console.log("Caller testCall " + method + ", " + data);
                reject(new Error("function input parameter error"));
                return;
            }

            if (this.releaseCall == true) {
                console.log("Caller testCall this.callee release");
                reject(new Error("Function inner object error"));
                return;
            }

            if (this.__call_obj__.callee == null) {
                console.log("Caller testCall this.callee is nullptr");
                reject(new Error("Function inner object error"));
                return;
            }

            console.log("Caller testCall msgData rpc.MessageParcel create");
            ...
            try {
                ...
                console.log("Caller testCall msgData rpc.SendRequest called");
                if (retData.errCode != 0) {
                    msgData.reclaim();
                    msgReply.reclaim();
                    console.log("Caller testCall return status " + status);
                    reject(new Error("Function execution exception"));
                    return;
                }
            } catch (e) {
                console.log("Caller testCall msgData rpc.MessageParcel error " + e);
            }
            ...
            try {
                ...
                if (retval === REQUEST_SUCCESS && str === 'object') {
                    console.log("Caller testCall return str " + str);
                    msgData.reclaim();
                    msgReply.reclaim();
                } else {
                    console.log("Caller testCall retval is [" + retval + "], str [" + str + "]");
                    msgData.reclaim();
                    msgReply.reclaim();
                ...
                console.log("Caller testCall msgData SendRequest retval error");
                reject(new Error("Function execution result is abnormal"));
                return;
            }

            console.log("Caller testCall msgData SendRequest end");
            resolve(undefined);
            return;
        });
    }
