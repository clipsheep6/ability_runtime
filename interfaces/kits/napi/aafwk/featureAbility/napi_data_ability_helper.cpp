...
void ExecuteBatchExecuteCB(napi_env env, void *data)
{
    ...
    if (executeBatchCB->dataAbilityHelper != nullptr) {
        ...
        HILOG_INFO("%{public}s, dataAbilityHelper is not nullptr. %{public}zu", __func__, executeBatchCB->result.size());
    }
...
{
    HILOG_INFO("%{public}s, NAPI_ExecuteBatch, getDataAbilityResultForResult start. %{public}zu", __func__, dataAbilityResult.size());
    int32_t index = 0;
