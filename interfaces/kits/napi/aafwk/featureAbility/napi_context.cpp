...
void GetOrCreateLocalDirExecuteCB(napi_env env, void *data)
{
    ...
    string abilityName = getOrCreateLocalDirCB->cbBase.ability->GetAbilityInfo()->name;
    HILOG_INFO("NAPI_GetOrCreateLocalDir, abilityName:%{public}s.", abilityName.c_str());
    // getOrCreateLocalDirCB->rootDir = // TODO + CONTEXT_DEAL_FILE_SEPARATOR + abilityName;
 ...
 * @return Return JS data successfully, otherwise return nullptr.
 */
napi_value GetOrCreateLocalDirAsync(napi_env env, napi_value *args, size_t argcAsync, const size_t argcPromise,
    GetOrCreateLocalDirCB *getOrCreateLocalDirCB)
{
    HILOG_INFO("%{public}s, asyncCallback.", __func__);
    ...
    if (valuetype == napi_function) {
        NAPI_CALL(
            env, napi_create_reference(env, args[argcPromise], 1, &getOrCreateLocalDirCB->cbBase.cbInfo.callback));
    }
    NAPI_CALL(env,
        napi_create_async_work(env,
            nullptr,
...
napi_value GetOrCreateLocalDirWrap(napi_env env, napi_callback_info info, GetOrCreateLocalDirCB *getOrCreateLocalDirCB)
{
    ...
    if (argcAsync > argcPromise) {
        ret = GetOrCreateLocalDirAsync(env, args, argcAsync, argcPromise, getOrCreateLocalDirCB);
    } else {
