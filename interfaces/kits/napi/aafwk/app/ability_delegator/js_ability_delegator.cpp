    ...
    NativeEngine &engine, NativeValue *value, std::shared_ptr<AbilityMonitor> &monitor)
{
    ...
    std::shared_ptr<JSAbilityMonitor> abilityMonitor(new JSAbilityMonitor(abilityName));
    ...
    NativeEngine &engine, NativeValue *value, sptr<OHOS::IRemoteObject> &remoteObject)
{
    ...
    for (auto iter = ablityRecord.begin(); iter != ablityRecord.end(); ++iter) {
        if (value->StrictEquals(iter->first->Get())) {
            ...
            return remoteObject ? result : nullptr;
        }
    }

    HILOG_ERROR("Ablity isn't existed");
...
bool JSAbilityDelegator::ParseJSExecuteShellCommandSecondPara(
    NativeEngine &engine, NativeValue *value, int64_t &timeoutSecs)
{
