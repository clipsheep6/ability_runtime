...
#include "js_ability_delegator_info_utils.h"
...
std::vector<std::unique_ptr<NativeReference>> jsMonitor;
...
NativeValue *JSAbilityDelegator::OnAddAbilityMonitor(NativeEngine &engine, NativeCallbackInfo &info)
{
    ...
    bool isExist = false;
    std::unique_ptr<JSAbilityMonitor> abilityMonitor = nullptr;
    if (!ParseJSMonitorPara(engine, info.argv[INDEX_ZERO], monitor, isExist, abilityMonitor)) {
        return engine.CreateUndefined();
    }

    if (isExist) {
        jsMonitor.emplace_back(std::move(abilityMonitor->GetJsAbilityMonitor()));
    }
...
NativeValue *JSAbilityDelegator::OnRemoveAbilityMonitor(NativeEngine &engine, NativeCallbackInfo &info)
{
    ...
    std::shared_ptr<AbilityMonitor> monitor;
    bool isExist = false;
    std::unique_ptr<JSAbilityMonitor> abilityMonitor = nullptr;
    if (!ParseJSMonitorPara(engine, info.argv[INDEX_ZERO], monitor, isExist, abilityMonitor)) {
        return engine.CreateUndefined();
    ...
    AsyncTask::Schedule(engine, CreateAsyncTaskWithLastParam(engine, lastParam, nullptr, std::move(complete), &result));
    if (isExist) {
        for (auto iter = jsMonitor.begin(); iter != jsMonitor.end(); ++iter) {
            if ((info.argv[INDEX_ZERO])->StrictEquals((*iter)->Get())) {
                iter = jsMonitor.erase(iter);
            } else {
                ++iter;
            }
        }
...
NativeValue *JSAbilityDelegator::OnWaitAbilityMonitor(NativeEngine &engine, NativeCallbackInfo &info)
{
    ...
    AsyncTask::CompleteCallback complete =
        [argcnum, monitor, timeout, hasTimeoutPara](NativeEngine &engine, AsyncTask &task, int32_t status) {
            ...
            task.Resolve(engine, CreateJsAbility(engine, remoteObject));
        };
...
napi_value JSAbilityDelegator::ParseJSMonitorPara(
    NativeEngine &engine, NativeValue *value, std::shared_ptr<AbilityMonitor> &monitor, 
    bool &isExist, std::unique_ptr<JSAbilityMonitor> &abilityMonitor)
{
    HILOG_INFO("ParseJSMonitorPara is called");

    for (auto iter = jsMonitor.begin(); iter != jsMonitor.end(); ++iter) {
        if(value->StrictEquals((*iter)->Get())) {
            HILOG_ERROR("monitor is existed");
            isExist = false;
            return nullptr;
        } else {
            isExist = true;
    ...
    std::string abilityName = GetString(abilityNameValue);

    abilityMonitor = std::make_unique<JSAbilityMonitor>(abilityName, &engine);
    if (!abilityMonitor) {
        HILOG_INFO("Failed to create JSAbilityMonitor");
        return nullptr;
    }
    object->SetNativePointer(abilityMonitor.release(), JSAbilityMonitor::Finalizer, nullptr);
...
std::string JSAbilityDelegator::GetString(NativeValue *value)
{
    HILOG_INFO("GetString is called");
        ...
        NativeEngine &engine, NativeCallbackInfo &info, std::string &cmd, int64_t &timeoutSecs, bool &hasTimeoutPara)
{
    ...
    if (!ParseJSExecuteShellCommandFirstPara(engine, info.argv[0], cmd)) {
            return engine.CreateUndefined();
    }

    if ((((info.argc == ARGC_TWO) || (info.argc == ARGC_THREE)))
        && ((info.argv[ARGC_ONE])->TypeOf() == NativeValueType::NATIVE_NUMBER)) {
        if (!ParseJSExecuteShellCommandSecondPara(engine, info.argv[ARGC_ONE], timeoutSecs)) {
            return engine.CreateUndefined();
        }
        hasTimeoutPara = true;
    }
...
bool JSAbilityDelegator::ParseJSExecuteShellCommandFirstPara(NativeEngine &engine, NativeValue *value, std::string &cmd)
{
    HILOG_INFO("ParseJSExecuteShellCommandFirstPara is called");

    if (!ConvertFromJsValue(engine, value, cmd)) {
...
bool JSAbilityDelegator::ParseJSExecuteShellCommandSecondPara(NativeEngine &engine, NativeValue *value, int64_t &timeoutSecs)
{
    HILOG_INFO("ParseJSExecuteShellCommandSecondPara is called");

    if (!ConvertFromJsValue(engine, value, timeoutSecs)) {
