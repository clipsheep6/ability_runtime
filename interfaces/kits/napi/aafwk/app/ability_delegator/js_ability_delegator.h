...
private:
    napi_value ParseJSMonitorPara(NativeEngine &engine, NativeValue *value, std::shared_ptr<AbilityMonitor> &monitor,
        bool &isExist, std::unique_ptr<JSAbilityMonitor> &abilityMonitor);
    napi_value ParseJSAbilityPara(NativeEngine &engine, NativeValue *value, sptr<OHOS::IRemoteObject> &remoteObject);
    ...
    bool ParseJSExecuteShellCommandFirstPara(NativeEngine &engine, NativeValue *value, std::string &cmd);
    bool ParseJSExecuteShellCommandSecondPara(NativeEngine &engine, NativeValue *value, int64_t &timeoutSecs);
    void SetJsAbilityMonitor(std::string jstring);
    std::string GetString(NativeValue* value);
};
}  // namespace AbilityDelegatorJs
}  // namespace OHOS
