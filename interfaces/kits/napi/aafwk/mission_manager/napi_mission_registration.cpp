...
{
    HILOGI("%{public}s call.", __func__);
    bool isFixConflict = false;
    napi_has_named_property(env, value, "fixConflict", &isFixConflict);
    if (!isFixConflict) {
        HILOGE("%{public}s, Wrong argument name for fixConflict.", __func__);
        return false;
    }
    ...
    if (fixConflictValue == nullptr) {
        HILOGE("%{public}s, not find fixConflict.", __func__);
        return false;
    }
    napi_valuetype valueType = napi_undefined;
    napi_typeof(env, fixConflictValue, &valueType);
    if (valueType != napi_boolean) {
        HILOGE("%{public}s, fixConflict error type.", __func__);
        return false;
    }
    bool isTag = false;
    napi_has_named_property(env, value, "tag", &isTag);
    if (!isTag) {
        HILOGE("%{public}s, Wrong argument name for tag.", __func__);
        return false;
    }
    napi_value tagValue = nullptr;
    napi_get_named_property(env, value, "tag", &tagValue);
    if (tagValue == nullptr) {
        HILOGE("%{public}s, not find tag.", __func__);
        return false;
    }
    napi_typeof(env, tagValue, &valueType);
    if (valueType != napi_number) {
        HILOGE("%{public}s, tag error type.", __func__);
        return false;
    }
    ...
    bool isStart, SyncRemoteMissionsContext* context)
{
    ...
    if (valueType != napi_object) {
        HILOGE("%{public}s, Wrong argument type.", __func__);
        return false;
    }
    ...
    if (!isDeviceId) {
        HILOGE("%{public}s, Wrong argument name for deviceId.", __func__);
        return false;
    }
    napi_get_named_property(env, value, "deviceId", &deviceIdValue);
    if (deviceIdValue == nullptr) {
        HILOGE("%{public}s, not find deviceId.", __func__);
        return false;
    }
    napi_typeof(env, deviceIdValue, &valueType);
    if (valueType != napi_string) {
        HILOGE("%{public}s, deviceId error type.", __func__);
        return false;
    }
    napi_get_value_string_utf16(env, deviceIdValue, context->deviceId, VALUE_BUFFER_SIZE, &context->valueLen);
    if (isStart && !SetStartSyncMissionsContext (env, value, context)) {
        HILOGE("%{public}s, Wrong argument for start sync.", __func__);
        return false;
    }
    HILOGI("%{public}s end.", __func__);
    return true;
    ...
    SyncRemoteMissionsContext* syncContext)
{
    ...
    if (argc != ARGS_ONE && argc != ARGS_TWO) {
        HILOGE("%{public}s, argument size error.", __func__);
        return false;
    }
    syncContext->env = env;
    if (!SetSyncRemoteMissionsContext(env, argv[0], isStart, syncContext)) {
        HILOGE("%{public}s, Wrong argument.", __func__);
        return false;
    }
    if (argc == ARGS_TWO) {
        napi_valuetype valueType = napi_undefined;
        ...
        if (valueType != napi_function) {
            HILOGE("%{public}s, callback error type.", __func__);
            return false;
        }
...
RegisterMissonCB *CreateRegisterMissonCBCBInfo(napi_env env)
{
    ...
    if (registerMissonCB == nullptr) {
        HILOGE("%{public}s registerMissonCB == nullptr", __func__);
        return nullptr;
    }
...
void RegisterMissonExecuteCB(napi_env env, void *data)
{
    ...
    HILOGI("set callback success.");

    registerMissonCB->result = -1;
    // registerMissonCB->result =
    //     AbilityManagerClient::GetInstance()->
    //     RegisterMissionListener(registerMissonCB->deviceId.c_str(),
    //     registerMissonCB->missionRegistration);
    // std::lock_guard<std::mutex> autoLock(registrationLock_);
    // if (registerMissonCB->result == NO_ERROR) {
    //     HILOGI("add registration.");
...
{
    HILOGI("%{public}s asyncCallback.", __func__);
    if (args == nullptr || registerMissonCB == nullptr) {
        HILOGE("%{public}s, param == nullptr.", __func__);
        return nullptr;
    }
    napi_value result = nullptr;
    ...
    HILOGI("%{public}s asyncCallback end.", __func__);
    return result;
}
...
bool SetCallbackReference(napi_env env, const napi_value& value, RegisterMissonCB *registerMissonCB)
{
    ...
    if (!isFirstCallback || !isSecondCallback || !isThirdCallback) {
        HILOGE("%{public}s, Wrong argument name for callback.", __func__);
        return false;
    }
    napi_value jsMethod = nullptr;
    napi_get_named_property(env, value, "notifyMissionsChanged", &jsMethod);
    if (jsMethod == nullptr) {
        HILOGE("%{public}s, not find callback notifyMissionsChanged.", __func__);
        return false;
    }
    napi_valuetype valuetype = napi_undefined;
    napi_typeof(env, jsMethod, &valuetype);
    if (valuetype != napi_function) {
        HILOGE("%{public}s, notifyMissionsChanged callback error type.", __func__);
        return false;
    }
    ...
    if (jsMethod == nullptr) {
        HILOGE("%{public}s, not find callback notifySnapshot.", __func__);
        return false;
    }
    napi_typeof(env, jsMethod, &valuetype);
    if (valuetype != napi_function) {
        HILOGE("%{public}s, notifySnapshot callback error type.", __func__);
        return false;
    }
    ...
    if (jsMethod == nullptr) {
        HILOGE("%{public}s, not find callback notifyNetDisconnect.", __func__);
        return false;
    }
    napi_typeof(env, jsMethod, &valuetype);
    if (valuetype != napi_function) {
        HILOGE("%{public}s, notifyNetDisconnect callback error type.", __func__);
        return false;
    }
    napi_create_reference(env, jsMethod, 1,
        &registerMissonCB->missionRegistrationCB.callback[2]); // 2 refers the second argument
    HILOGI("%{public}s called end.", __func__);
    return true;
}
...
bool CreateCallbackReference(napi_env env, const napi_value& value, RegisterMissonCB *registerMissonCB)
{
    ...
    if (valuetype == napi_object) {
        if (!SetCallbackReference(env, value, registerMissonCB)) {
            HILOGE("%{public}s, Wrong callback.", __func__);
            return false;
        }
    } else {
        HILOGE("%{public}s, Wrong argument type.", __func__);
...
napi_value RegisterMissonWrap(napi_env env, napi_callback_info info, RegisterMissonCB *registerMissonCB)
{
    ...
    HILOGI("argcAsync is %{public}zu", argcAsync);
    if (argcAsync != ARGS_TWO && argcAsync != ARGS_THREE) {
        HILOGE("%{public}s, Wrong argument count.", __func__);
        return nullptr;
    }
    ...
    HILOGI("%{public}s deviceId:%{public}s", __func__, Str16ToStr8(registerMissonCB->deviceId).c_str());

    if (argcAsync > 1 && !CreateCallbackReference(env, args[1], registerMissonCB)) {
        HILOGE("%{public}s, Wrong arguments.", __func__);
        return nullptr;
    }
...
napi_value NAPI_RegisterMissionListener(napi_env env, napi_callback_info info)
{
    ...
    if (ret == nullptr) {
        HILOGE("%{public}s ret == nullptr", __func__);
        if (registerMissonCB != nullptr) {
            delete registerMissonCB;
            registerMissonCB = nullptr;
        }
...
void NAPIMissionRegistration::SetEnv(const napi_env &env)
{
    env_ = env;
}

void NAPIMissionRegistration::SetNotifyMissionsChangedCBRef(const napi_ref &ref)
{
    notifyMissionsChangedRef_ = ref;
}

void NAPIMissionRegistration::SetNotifySnapshotCBRef(const napi_ref &ref)
{
    notifySnapshotRef_ = ref;
}

void NAPIMissionRegistration::SetNotifyNetDisconnectCBRef(const napi_ref &ref)
{
...
{
    HILOGI("UvWorkNotifyMissionChanged, uv_queue_work");
    if (work == nullptr) {
        HILOGE("UvWorkNotifyMissionChanged, work is null");
        return;
    }
    RegisterMissonCB *registerMissonCB = static_cast<RegisterMissonCB *>(work->data);
    if (registerMissonCB == nullptr) {
        HILOGE("UvWorkNotifyMissionChanged, registerMissonCB is null");
        return;
    }
    napi_value result = nullptr;
    HILOGI("UvWorkNotifyMissionChanged, deviceId = %{public}s", Str16ToStr8(registerMissonCB->deviceId).c_str());
    result =
    ...
    HILOGI("UvWorkNotifyMissionChanged, uv_queue_work end");
}

void NAPIMissionRegistration::NotifyMissionsChanged(const std::u16string deviceId)
{
    HILOGI("%{public}s, called. deviceId = %{public}s", __func__, Str16ToStr8(deviceId).c_str());
    uv_loop_s *loop = nullptr;

    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
        HILOGE("%{public}s, loop == nullptr.", __func__);
        return;
    }

    uv_work_t *work = new uv_work_t;
    if (work == nullptr) {
        HILOGE("%{public}s, work==nullptr.", __func__);
        return;
    }
...
{
    HILOGI("UvWorkNotifySnapshot, uv_queue_work");
    if (work == nullptr) {
        HILOGE("UvWorkNotifySnapshot, work is null");
        return;
    }
    RegisterMissonCB *registerMissonCB = static_cast<RegisterMissonCB *>(work->data);
    if (registerMissonCB == nullptr) {
        HILOGE("UvWorkNotifySnapshot, registerMissonCB is null");
        return;
    }
    napi_value result[2] = {0};
    HILOGI("UvWorkNotifySnapshot, deviceId = %{public}s", Str16ToStr8(registerMissonCB->deviceId).c_str());
    result[0] =
        WrapString(registerMissonCB->cbBase.cbInfo.env, Str16ToStr8(registerMissonCB->deviceId).c_str(), "deviceId");
    HILOGI("UvWorkNotifySnapshot, missionId = %{public}d", registerMissonCB->missionId);
    result[1] =
    ...
    HILOGI("UvWorkNotifySnapshot, uv_queue_work end");
}

void NAPIMissionRegistration::NotifySnapshot(const std::u16string deviceId, int32_t missionId)
{
    HILOGI("%{public}s, called. deviceId = %{public}s", __func__, Str16ToStr8(deviceId).c_str());
    uv_loop_s *loop = nullptr;
...
{
    HILOGI("UvWorkNotifyNetDisconnect, uv_queue_work");
    if (work == nullptr) {
        HILOGE("UvWorkNotifyNetDisconnect, work is null");
        return;
    }
    RegisterMissonCB *registerMissonCB = static_cast<RegisterMissonCB *>(work->data);
    if (registerMissonCB == nullptr) {
        HILOGE("UvWorkNotifyNetDisconnect, registerMissonCB is null");
        return;
    }
    napi_value result[2] = {0};
    HILOGI("UvWorkNotifyNetDisconnect, deviceId = %{public}s", Str16ToStr8(registerMissonCB->deviceId).c_str());
    result[0] =
        WrapString(registerMissonCB->cbBase.cbInfo.env, Str16ToStr8(registerMissonCB->deviceId).c_str(), "deviceId");
    HILOGI("UvWorkNotifyNetDisconnect, state = %{public}d", registerMissonCB->state);
    result[1] =
    ...
    HILOGI("UvWorkNotifyNetDisconnect, uv_queue_work end");
}

void NAPIMissionRegistration::NotifyNetDisconnect(const std::u16string deviceId, int32_t state)
{
    HILOGI("%{public}s called. deviceId = %{public}s. state = %{public}d", __func__,
...
void UnRegisterMissonExecuteCB(napi_env env, void *data)
{
    ...
    // registerMissonCB->result =
    //     AbilityManagerClient::GetInstance()->
    //     UnRegisterMissionListener(registerMissonCB->deviceId.c_str(),
    //     registerMissonCB->missionRegistration);
    // if (registerMissonCB->result == NO_ERROR) {
    //     HILOGI("remove registration.");
...
napi_value WrapString(napi_env env, const std::string& param, const std::string& paramName)
{
    ...
    HILOGI("%{public}s called. %{public}s = %{public}s", __func__, paramName.c_str(), param.c_str());
...
napi_value WrapInt32(napi_env env, int32_t num, const std::string& paramName)
{
    ...
    napi_value jsValue = nullptr;
    HILOGI("%{public}s called. %{public}s = %{public}d", __func__, paramName.c_str(), num);
...
ContinueAbilityCB *CreateContinueAbilityCBCBInfo(napi_env env)
{
    ...
    if (continueAbilityCB == nullptr) {
        HILOGE("%{public}s continueAbilityCB == nullptr", __func__);
        return nullptr;
    }
...
void ContinueAbilityExecuteCB(napi_env env, void *data)
{
    ...
    HILOGI("create continueAbilityCB success.");
    sptr<NAPIMissionRegistration> continuation(new (std::nothrow) NAPIMissionRegistration());
    continueAbilityCB->abilityContinuation = continuation;
    if (continueAbilityCB->abilityContinuation == nullptr) {
        HILOGE("%{public}s abilityContinuation == nullptr.", __func__);
        return;
    }
    continueAbilityCB->abilityContinuation->SetContinueAbilityEnv(env);
    HILOGI("set env success.");
    ...
    HILOGI("%{public}s end. error:%{public}d ", __func__, continueAbilityCB->result);
}
...
bool CheckContinueFirstArgs(napi_env env, const napi_value &value, ContinueAbilityCB *continueAbilityCB)
{
    ...
    if (!isSrcDeviceId && !isDstDeviceId && !isMissionId && !isWantParam) {
        HILOGE("%{public}s, Wrong argument key.", __func__);
        return false;
    }
    napi_value firstNApi = nullptr;
    ...
    if (firstNApi == nullptr || secondNApi == nullptr || thirdNApi == nullptr || fourthNApi == nullptr) {
        HILOGE("%{public}s, miss required parameters.", __func__);
        return false;
    }
    napi_typeof(env, firstNApi, &valueType);
    if (valueType != napi_string) {
        HILOGE("%{public}s, Wrong argument type srcDeviceId.", __func__);
        return false;
    }
    ...
    if (valueType == napi_string) {
        ...
        HILOGI("dstDeviceId is %{public}s", continueAbilityCB->dstDeviceId.c_str());
    } else {
        HILOGE("%{public}s, Wrong argument type dstDeviceId.", __func__);
        return false;
    }
    napi_typeof(env, thirdNApi, &valueType);
    if (valueType != napi_number) {
        HILOGE("%{public}s, Wrong argument type missionId.", __func__);
        return false;
    }
    continueAbilityCB->missionId = AppExecFwk::UnwrapInt32FromJS(env, thirdNApi, -1);
    napi_typeof(env, fourthNApi, &valueType);
    if (valueType != napi_object) {
        HILOGE("%{public}s, Wrong argument type wantParam.", __func__);
        return false;
    }
...
napi_value ContinueAbilityWrap(napi_env env, napi_callback_info info, ContinueAbilityCB *continueAbilityCB)
{
    ...
    if (!CheckContinueFirstArgs(env, args[0], continueAbilityCB)) {
        HILOGE("%{public}s, Wrong first arguments.", __func__);
        return nullptr;
    }
    ...
    if (argcAsync > 1) {
        ...
        if (valuetype == napi_object) {
            bool isFirstCallback = false;
            napi_has_named_property(env, args[1], "onContinueDone", &isFirstCallback);
            if (!isFirstCallback) {
                HILOGE("%{public}s, Wrong argument name for onContinueDone.", __func__);
                return nullptr;
            }
            napi_get_named_property(env, args[1], "onContinueDone", &jsMethod);
            if (jsMethod == nullptr) {
                HILOGE("%{public}s, not find callback onContinueDone.", __func__);
                return nullptr;
            }
            napi_typeof(env, jsMethod, &valuetype);
            if (valuetype != napi_function) {
                HILOGE("%{public}s, onContinueDone callback error type.", __func__);
                return nullptr;
            }
...
{
    HILOGI("UvWorkOnCountinueDone, uv_queue_work");
    if (work == nullptr) {
        HILOGE("UvWorkOnCountinueDone, work is null");
        return;
    }
    ContinueAbilityCB *continueAbilityCB = static_cast<ContinueAbilityCB *>(work->data);
    if (continueAbilityCB == nullptr) {
        HILOGE("UvWorkOnCountinueDone, continueAbilityCB is null");
        return;
    }
    napi_value result = nullptr;
    HILOGI("UvWorkOnCountinueDone, resultCode = %{public}d", continueAbilityCB->resultCode);
    result =
    ...
    HILOGI("UvWorkOnCountinueDone, uv_queue_work end");
}

void NAPIMissionRegistration::OnContinueDone(int32_t result)
{
    HILOGI("%{public}s, called. result = %{public}d", __func__, result);
    uv_loop_s *loop = nullptr;
...
void NAPIMissionRegistration::SetContinueAbilityEnv(const napi_env &env)
{
    env_ = env;
}

void NAPIMissionRegistration::SetContinueAbilityCBRef(const napi_ref &ref)
{
