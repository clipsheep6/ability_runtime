...
#ifndef OHOS_ABILITY_RUNTIME_UIEXTENSION_MANAGER_PROXY_H
#define OHOS_ABILITY_RUNTIME_UIEXTENSION_MANAGER_PROXY_H

#include "iremote_proxy.h"
#include "session_info.h"
#include "uiextension_manager_interface.h"
...
/**
 * @class UIExtensionManagerProxy
 * UIExtensionManagerProxy.
 */
class UIExtensionManagerProxy : public IRemoteProxy<IUIExtensionManager> {
public:
    explicit UIExtensionManagerProxy(const sptr<IRemoteObject> &impl) : IRemoteProxy<IUIExtensionManager>(impl) {}
    virtual ~UIExtensionManagerProxy() = default;
    /**
    ...
    ErrCode StartUIExtensionAbility(
        const sptr<SessionInfo> &extensionSessionInfo, int32_t userId = DEFAULT_INVAL_VALUE) override;
     ...
     * @return Returns ERR_OK on success, others on failure.
     */
    ErrCode MinimizeUIExtensionAbility(const sptr<SessionInfo> &extensionSessionInfo, bool fromUser = false) override;
     ...
     * @return Returns ERR_OK on success, others on failure.
     */
    ErrCode CheckUIExtensionIsFocused(uint32_t uiExtensionTokenId, bool &isFocused) override;
...
private:
    static inline BrokerDelegator<UIExtensionManagerProxy> delegator_;
};
} // namespace AAFwk
} // namespace OHOS
#endif // OHOS_ABILITY_RUNTIME_UIEXTENSION_MANAGER_PROXY_H