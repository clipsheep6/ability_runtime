...
{
    TAG_LOGD(AAFwkTag::AUTOFILLMGR, "Called.");
    callback.onResult = [extensionCallback](int32_t errCode, const AAFwk::Want& want){ extensionCallback->OnResult(errCode, want); };  

    callback.onRelease = [extensionCallback](int arg1){ extensionCallback->OnRelease(arg1); };
    ...
    callback.onReceive = [extensionCallback](const AAFwk::WantParams &arg1)  { extensionCallback->OnReceive(arg1); };
