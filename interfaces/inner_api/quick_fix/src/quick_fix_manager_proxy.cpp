...
int32_t QuickFixManagerProxy::GetApplyedQuickFixInfo(const std::string &bundleName, AppExecFwk::AppqfInfo &quickFixInfo)
{
    HITRACE_METER_NAME(HITRACE_TAG_ABILITY_MANAGER, __PRETTY_FUNCTION__);
    HILOG_INFO("%{public}s is called.", __func__);
    ...
    if (result == QUICK_FIX_OK) {
        std::unique_ptr<AppExecFwk::AppqfInfo> info(reply.ReadParcelable<AppExecFwk::AppqfInfo>());
        if (info != nullptr) {
            quickFixInfo = *info;
        }
    }

    HILOG_INFO("%{public}s is finished with %{public}d.", __func__, result);
    return result;
}
}  // namespace AAFwk
}  // namespace OHOS
