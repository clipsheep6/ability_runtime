...
namespace OHOS {
namespace AAFwk {
void QuickFixManagerClient::ApplyQuickFix(const std::vector<std::string> &quickFixFiles)
{
    HILOG_DEBUG("ApplyQuickFix is called.");
    auto quickFixMgr = ConnectQuickFixManagerService();
    if (quickFixMgr) {
        quickFixMgr->ApplyQuickFix(quickFixFiles);
    }
}

sptr<IQuickFixManager> QuickFixManagerClient::ConnectQuickFixManagerService()
{
    HILOG_DEBUG("ConnectQuickFixManagerService is called.");
    ...
    if (quickFixMgr_ == nullptr) {
        ...
        sptr<QfmsDeathRecipient> recipient(new QfmsDeathRecipient(onClearProxyCallback));
        quickFixMgr_->AsObject()->AddDeathRecipient(recipient);
    }

    HILOG_DEBUG("ConnectQuickFixManagerService is finished.");
    return quickFixMgr_;
}

void QuickFixManagerClient::ClearProxy()
{
    HILOG_DEBUG("ClearProxy is called.");
    std::lock_guard<std::mutex> lock(mutex_);
...
{
    HILOG_ERROR("quick fix manager service stub died.");
    proxy_();
}
}  // namespace AAFwk
}  // namespace OHOS
