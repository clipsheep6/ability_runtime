...
void StartSpecifiedAbilityResponseProxy::OnStartSpecifiedProcessResponse(const AAFwk::Want &want, const std::string &flag)
{
    ...
    int32_t ret = remote->SendRequest(
        static_cast<uint32_t>(IStartSpecifiedAbilityResponse::Message::ON_START_SPECIFIED_PROCESS_RESPONSE),
        data, reply, option);
    if (ret != NO_ERROR) {
        HILOG_WARN("SendRequest is failed, error code: %{public}d", ret);
    }
...
void StartSpecifiedAbilityResponseProxy::OnStartSpecifiedProcessTimeoutResponse(const AAFwk::Want &want)
{
    ...
    int32_t ret = remote->SendRequest(static_cast<uint32_t>(
        IStartSpecifiedAbilityResponse::Message::ON_START_SPECIFIED_PROCESS_TIMEOUT_RESPONSE),
        data, reply, option);
