...
bool ExitResidentProcessInfo::GetExitResidentBundleNames(std::vector<std::string> &bundleNames)
{
    if (exitResidentBundleNames_.empty()) {
        TAG_LOGE(AAFwkTag::APPMGR, "no Resident Process exit");
        return false;
    }
...
ffrt::mutex &ExitResidentProcessInfo::GetMutexLock()
{
    return mutexLock_;
}
}  // namespace AAFwk
