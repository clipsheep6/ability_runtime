...
AppMgrStub::AppMgrStub()
{
    ...
    memberFuncMap_[static_cast<uint32_t>(AppMgrInterfaceCode::APP_GET_CURRENT_APP_TWIN_INDEX)] =
        &AppMgrStub::HandleGetCurrentAppTwinIndex;
...
int32_t AppMgrStub::HandleGetCurrentAppTwinIndex(MessageParcel &data, MessageParcel &reply)
{
    HITRACE_METER(HITRACE_TAG_APP);
    int32_t appIndex;
    auto result = GetCurrentAppTwinIndex(appIndex);
    if (!reply.WriteInt32(result)) {
        return ERR_INVALID_VALUE;
    }
