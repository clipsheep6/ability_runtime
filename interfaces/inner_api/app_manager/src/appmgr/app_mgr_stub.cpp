...
AppMgrStub::AppMgrStub()
{
    ...
    memberFuncMap_[static_cast<uint32_t>(AppMgrInterfaceCode::SCHEDULE_START_SPECIFIED_PROCESS_DONE)] =
        &AppMgrStub::HandleScheduleStartSpecifiedProcessDone;
...
int32_t AppMgrStub::HandleScheduleStartSpecifiedProcessDone(MessageParcel &data, MessageParcel &reply)
{
    auto recordId = data.ReadInt32();
    AAFwk::Want *want = data.ReadParcelable<AAFwk::Want>();
    if (want == nullptr) {
    ...
    ScheduleStartSpecifiedProcessDone(recordId, *want, flag);
    delete want;
    return NO_ERROR;
}
