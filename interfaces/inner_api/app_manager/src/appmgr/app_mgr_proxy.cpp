...
void AppMgrProxy::ScheduleStartSpecifiedProcessDone(const int32_t recordId, const AAFwk::Want &want, const std::string &flag)
{
    MessageParcel data;
    MessageParcel reply;
    if (!WriteInterfaceToken(data)) {
    ...
    if (!SendTransactCmd(AppMgrInterfaceCode::SCHEDULE_START_SPECIFIED_PROCESS_DONE, data, reply)) {
        HILOG_ERROR("SendTransactCmd failed");
        return;
    }
}
