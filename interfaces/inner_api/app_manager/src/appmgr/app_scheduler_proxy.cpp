...
void AppSchedulerProxy::ScheduleStartSpecifiedProcess(const AAFwk::Want &want, const std::string &moduleName)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_ASYNC);
    ...
    int32_t ret = remote->SendRequest(
        static_cast<uint32_t>(IAppScheduler::Message::SCHEDULE_START_SPECIFIED_PROCESS), data, reply, option);
    if (ret != NO_ERROR) {
        HILOG_WARN("SendRequest is failed, error code: %{public}d", ret);
    }
}
...
{
    HILOG_DEBUG("Called.");
    SendRequest(IAppScheduler::Message::SCHEDULE_ATTACH_APP_DEBUG);
}

void AppSchedulerProxy::DetachAppDebug()
{
    HILOG_DEBUG("Called.");
    SendRequest(IAppScheduler::Message::SCHEDULE_DETACH_APP_DEBUG);
}

void AppSchedulerProxy::SendRequest(const IAppScheduler::Message &message)
{
    HILOG_DEBUG("Called.");
    MessageParcel data;
    if (!WriteInterfaceToken(data)) {
