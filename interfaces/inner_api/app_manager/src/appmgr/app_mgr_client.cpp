...
void AppMgrClient::ScheduleStartSpecifiedProcessDone(const int32_t recordId, const AAFwk::Want &want, const std::string &flag)
{
    sptr<IAppMgr> service = iface_cast<IAppMgr>(mgrHolder_->GetRemoteObject());
    if (service == nullptr) {
        HILOG_ERROR("service is nullptr");
    ...
    service->ScheduleStartSpecifiedProcessDone(recordId, want, flag);
}
