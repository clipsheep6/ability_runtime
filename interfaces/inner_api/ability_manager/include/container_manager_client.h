...
public:
    ...
    static std::shared_ptr<ContainderManagerClient> GetInstance();

    /**
     * Notify the state of book
     * 
     * @param state, state of book.
     * @reture Return ERR_OKon success, others on failure.
     */
    ErrCode NotifyBootComplete(int state);
private:
    /**
     * Connect ability manager srvice.
     * 
     * @reture Return ERR_Ok on success, others on failure.
     */
    ErrCode Connect();
