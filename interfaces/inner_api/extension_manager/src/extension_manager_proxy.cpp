...
int ExtensionManagerProxy::ConnectAbility(const Want &want, const sptr<IRemoteObject> &connect,
    const sptr<IRemoteObject> &callerToken, int32_t userId)
{
    int32_t serviceExtensionType = 3;
    return ConnectAbilityCommon(want, connect, callerToken, serviceExtensionType, userId);
}

int ExtensionManagerProxy::ConnectAbilityCommon(const Want &want, const sptr<IRemoteObject> &connect,
    const sptr<IRemoteObject> &callerToken, int32_t extensionType, int32_t userId)
{
    if (connect == nullptr) {
