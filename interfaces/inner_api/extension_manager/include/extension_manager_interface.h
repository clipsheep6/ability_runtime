...
public:
     ...
     */
    virtual int ConnectAbility(const Want &want, const sptr<IRemoteObject> &connect,
        const sptr<IRemoteObject> &callerToken, int32_t userId = DEFAULT_INVALID_USER_ID) = 0;
    ...
    virtual int ConnectAbilityCommon(const Want &want, const sptr<IRemoteObject> &connect,
        const sptr<IRemoteObject> &callerToken, int32_t extensionType, int32_t userId = DEFAULT_INVALID_USER_ID)
    {
        return 0;
    }
