...
public:
     ...
     * @return Returns ERR_OK on success, others on failure.
     */
    virtual int FreeInstallAbility(const Want &want, const sptr<IRemoteObject> &callerToken,
        const sptr<IRemoteObject> &statusCallback, int requestCode = DEFAULT_INVAL_VALUE,
        int32_t userId = DEFAULT_INVAL_VALUE)
    {
        return 0;
    }
