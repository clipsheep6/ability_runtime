...
#ifndef OHOS_AAFWK_MY_SA_MANAGER_CLIENT_H
#define OHOS_AAFWK_MY_SA_MANAGER_CLIENT_H

#include <functional>

#include "singleton.h"
#include "uri.h"
#include "my_sa_manager_interface.h"

namespace OHOS {
namespace AAFwk {
using ClearProxyCallback = std::function<void()>;
class MySaManagerClient : public DelayedSingleton<MySaManagerClient>,
                                   public std::enable_shared_from_this<MySaManagerClient> {
public:
    MySaManagerClient() = default;
    ~MySaManagerClient() = default;
...
private:
    sptr<IMySaManager> ConnectUriPermService();
    void ClearProxy();
    DISALLOW_COPY_AND_MOVE(MySaManagerClient);
...
private:
    std::mutex mutex_;
    sptr<IMySaManager> uriPermMgr_ = nullptr;
};
}  // namespace AAFwk
}  // namespace OHOS
#endif  // OHOS_AAFWK_MY_SA_MANAGER_CLIENT_H