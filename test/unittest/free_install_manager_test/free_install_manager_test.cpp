...
HWTEST_F(FreeInstallTest, FreeInstall_OnInstallFinished_Repeat_001, TestSize.Level1)
{
    Want want;
    ElementName element("", "com.test.demo", "MainAbility");
    want.SetElement(element);
    ...
    freeInstallManager_->freeInstallList_.resize(0);
    freeInstallManager_->freeInstallList_.emplace_back(info);
    freeInstallManager_->NotifyFreeInstallResult(want, 11, info.startInstallTime);
    HILOG_ERROR("[DongLin] size: %{public}zu", freeInstallManager_->freeInstallList_.size());
    freeInstallManager_->NotifyFreeInstallResult(want, 11, info.startInstallTime);
    HILOG_ERROR("[DongLin] size2: %{public}zu", freeInstallManager_->freeInstallList_.size());
}
}  // namespace AppExecFwk
}  // namespace OHOS