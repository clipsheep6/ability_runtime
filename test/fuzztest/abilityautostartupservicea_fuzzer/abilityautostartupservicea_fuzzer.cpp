...
bool DoSomethingInterestingWithMyAPI(const char* data, size_t size)
{
    ...
    service->RegisterAutoStartupSystemCallback(nullptr);
    service->RegisterAutoStartupSystemCallback(token);
    service->RegisterAutoStartupSystemCallback(token);
    service->UnregisterAutoStartupSystemCallback(token);
    ...
    service->DeleteAutoStartupData(info.bundleName, int32Param);
    return true;
}
}
