...
bool Zip(const ZipParams &params, const OPTIONS &options, CALLBACK callback)
{
    ...
    std::vector<FilePath> allFiles;
    if (filesToAdd->empty()) {
        // Include all files from the srcDir (modulo the srcDir itself and
        ...
        for (auto iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            if (iter != entries.begin() &&  // Don't filter the root dir.
                ((!params.GetIncludeHiddenFiles() && IsHiddenFile(constEntryPath)) ||
            ...
            if (iter != entries.begin()) {  // Exclude the root dir from the ZIP file.
                ...
                bool success = paramsSrcPath.AppendRelativePath(entryPath, &relative_path);
                if (success) {
                    allFiles.push_back(relative_path);
                }
            }
    ...
    if (params.DestFd() != kInvalidPlatformFile) {
        zip_writer = ZipWriter::CreateWithFd(params.DestFd(), params.SrcDir());
        if (!zip_writer)
            return false;
    }
    if (!zip_writer) {
        zip_writer = ZipWriter::Create(params.DestFile(), params.SrcDir());
        if (!zip_writer) {
            return false;
        }
    }
    return zip_writer->WriteEntries(*filesToAdd, options, callback);
}
...
bool UnzipWithFilterCallback(const FilePath &srcFile, const FilePath &destDir, const OPTIONS &options,
    CALLBACK callback, FilterCallback filterCB, bool log_skipped_files)
{
