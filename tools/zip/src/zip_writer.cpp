...
{
    HILOG_INFO("%{public}s called", __func__);
    std::string str_path = path.Value();
    return ZipOpenNewFileInZip(zip_file, str_path, options, last_modified);
}
...
bool AddFileEntryToZip(zipFile zip_file, FilePath &relative_path, FilePath &absolute_path, const OPTIONS &options)
{
    HILOG_INFO("%{public}s called", __func__);
    ...
    if (!OpenNewFileEntry(zip_file, relative_path, false, lastModified, options)) {
        return false;
    }
...
bool ZipWriter::FlushEntriesIfNeeded(bool force, const OPTIONS &options, CALLBACK callback)
{
    ...
    while (pendingEntries_.size() >= kMaxPendingEntriesCount || (force && !pendingEntries_.empty())) {
        ...
        for (size_t i = 0; i < absolutePaths.size(); i++) {
            FilePath &relative_path = relativePaths[i];
            FilePath &absolute_path = absolutePaths[i];
            if (FilePath::PathIsValid(absolute_path)) {
                if (!AddFileEntryToZip(zipFile_, relative_path, absolute_path, options)) {
                ...
                if (!AddDirectoryEntryToZip(zipFile_, relative_path, last_modified, options)) {
                    CALLING_CALL_BACK(callback, ERROR_CODE_ERRNO)
