...
class IsMockSaCall {
public:
    static void IsMockSaCallWithPermission(bool permissionFlag) {
        if (permissionFlag) {
            uint64_t tokenId;
            const char* perms[] = {
            ...
            infoInstance.processName = "MockSaCallWithPermission";
            ...
            infoInstance.processName = "MockSaCallWithNoPermission";
            tokenId = GetAccessTokenId(&infoInstance);
            SetSelfTokenID(tokenId);
            Security::AccessToken::AccessTokenKit::ReloadNativeTokenInfo();
        }
